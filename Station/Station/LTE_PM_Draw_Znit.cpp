...
IMPLEMENT_DYNCREATE(CLTE_PM_Draw_Znit, CFormView)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CLTE_PM_Draw_Znit::CLTE_PM_Draw_Znit()
	: CFormView(CLTE_PM_Draw_Znit::IDD)
{
	v_bShow=false;
}
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CLTE_PM_Draw_Znit, CFormView)
	ON_WM_DESTROY()
	ON_WM_SIZE()
	//Chart
	ON_WM_MOUSEWHEEL()
	...
	ON_MESSAGE(123,&CLTE_PM_Draw_Znit::Message_ChartView)					//添加消息映射 

	ON_NOTIFY(DTN_CLOSEUP, IDC_BEG_DATE, &CLTE_PM_Draw_Znit::OnDtnCloseupBegDate)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CLTE_PM_Draw_Znit::OnCbnSelchangeCombo1)
	ON_BN_CLICKED(IDC_REFRESH, &CLTE_PM_Draw_Znit::OnBnClickedRefresh)
	ON_BN_CLICKED(IDC_DELETE, &CLTE_PM_Draw_Znit::OnBnClickedDelete)
	//MutiTreCtrl：消息 [点击了State]
	ON_BN_CLICKED(IDC_CK_MUTI, &CLTE_PM_Draw_Znit::OnBnClickedCkMuti)
	ON_BN_CLICKED(IDC_QUARY, &CLTE_PM_Draw_Znit::OnBnClickedQuary)
	ON_NOTIFY(NM_CLICK, IDC_LIST1, &CLTE_PM_Draw_Znit::OnNMClickList1)
	ON_BN_CLICKED(IDC_INSTANCE_DETAIL, &CLTE_PM_Draw_Znit::OnBnClickedInstanceDetail)
	ON_BN_CLICKED(IDC_BUTTON1, &CLTE_PM_Draw_Znit::OnBnClickedButton1)
END_MESSAGE_MAP()


// CLTE_PM_Draw_Znit 诊断

#ifdef _DEBUG
void CLTE_PM_Draw_Znit::AssertValid() const
{
	CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CLTE_PM_Draw_Znit::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif
...
// CLTE_PM_Draw_Znit 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::OnInitialUpdate()
{
	CFormView::OnInitialUpdate();
	CString v_sTemp;													//临时变量
	//窗体名称
	v_sFrame_Name="LTE 性能图形查询_原始";								//窗体名称
	...
	m_SplitterPane_Main.Create(											//垂直线
		...
		IDC_LTE_PM_Draw_Znit_Main,		// this ID is used for saving/restoring splitter position and it must be unique !
...
//------------------------------------------------------------------------------------------------------			
BOOL CLTE_PM_Draw_Znit::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 在此添加专用代码和/或调用基类
...
//	函数功能：子窗销毁
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::OnDestroy()
{
	CFormView::OnDestroy();
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::OnSize(UINT nType, int cx, int cy)
{
...
//------------------------------------------------------------------------------------------------------
LRESULT CLTE_PM_Draw_Znit::Message_ChartView(WPARAM iParam1,LPARAM iParam2)
{
	//绘图：Chart
...
//	函数功能：点击列表窗 [小区]
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::OnNMClickList1(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：刷新【重新绘制曲线】
//------------------------------------------------------------------------------------------------------	
void CLTE_PM_Draw_Znit::OnBnClickedRefresh()
{
...
//	函数功能：小区查询
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::OnBnClickedQuary()
{
	// TODO: 在此添加控件通知处理程序代码
	if (v_pIterator_Frame->second.v_iThread_Exit==0)
	{
...
//	函数功能：性能选择
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::OnBnClickedInstanceDetail()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：性能导出
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CLTE_PM_Draw_Znit::My_Thread_Inoput(LPVOID lparam)
{
	CLTE_PM_Draw_Znit *  lp_this = NULL ;
	lp_this = (CLTE_PM_Draw_Znit *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 1:
		My_LoadData_Cell();												//查询Cell信息
		break;
	case 2:
		My_Chart();														//单线绘制
		break;
...
//	函数功能：装载Cell信息
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::My_LoadData_Cell()
{
	CString		v_sSql;													//临时变量
...
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::OnMouseMovePlotArea()
{
...
//------------------------------------------------------------------------------------------------------
BOOL CLTE_PM_Draw_Znit::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
...
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::OnViewPortChanged()
{
...
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::OnChart_Para()
{
	CString		v_sTemp;												//临时变量
...
//	函数功能：清除屏幕显示信息
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::My_Show_Clear()									    //清除屏幕显示信息
{
...
//	函数功能：点击多线
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::OnBnClickedCkMuti()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：清除图形 [绘图复位]
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::OnBnClickedDelete()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：绘图复位
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::My_Chart_New()
{
	//绘图初始化
	OnChart_Para();														//传递：绘图参数
	m_cChartView.initChartViewer(&m_cChartView);						//初始化Chart
...
//	函数功能：曲线绘制【单线、多线】
//------------------------------------------------------------------------------------------------------	
void CLTE_PM_Draw_Znit::My_Chart_Muti()
{
...
//	函数功能：绘图复位然后找当前节点
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::My_Chart()
{
	//绘图复位
	My_Chart_New();														//绘图复位
...
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::My_List_Chart(int v_iList_Item)
{
...
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::My_List_Chart_GetNode(int v_iList_Item)
{
...
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::My_Chart_IP()
{
	bool					v_bOK=false;								//临时变量
...
//	函数功能：日期选择
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::OnDtnCloseupBegDate(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：天数选择
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::OnCbnSelchangeCombo1()
{
	// TODO: 在此添加控件通知处理程序代码
...
//------------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::My_Load_Performance(CString v_sType,CString v_sFile_Key,CString v_sCell,CString v_sCell_ID,CString v_sPerformance,CString v_sField,COleDateTime v_dDate_Start,int v_iDay_Sum)
{
	CString			v_sSql,v_sTemp,v_sTime;								//临时变量
	CString			v_sTime_Start,v_sTime_End;							//日期
	int				v_iNum_Old=-1;										//记录：序号
//	int				v_iTable_Num;										//实时表序号
	...
	try
	{
		...
		//【2】从数据表中数据读取：1次读1个表的数据 [一个表为1月数据]
		int v_iDay=0;													//读取天数
...
//------------------------------------------------------------------------------------------------------			
CString CLTE_PM_Draw_Znit::My_Get_Table_Num(int v_iDay)
{
	CString	v_sNum;
...
//	函数功能：装载性能参数信息
//------------------------------------------------------------------------------------------------------			
void CLTE_PM_Draw_Znit::My_LoadData_Para()
{
	int				v_iID=0;											//临时变量
...
//-------------------------------------------------------------------------------------------------
void CLTE_PM_Draw_Znit::My_SendToFile() 
{
