...
IMPLEMENT_DYNCREATE(CCON_LTE_Cell, CFormView)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CCON_LTE_Cell::CCON_LTE_Cell()
	: CFormView(CCON_LTE_Cell::IDD)
{
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CCON_LTE_Cell, CFormView)
	ON_WM_DESTROY()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_QUARY, &CCON_LTE_Cell::OnBnClickedQuary)
	ON_BN_CLICKED(IDC_ELEMENT, &CCON_LTE_Cell::OnBnClickedElement)
	ON_BN_CLICKED(IDC_EXCEL2, &CCON_LTE_Cell::OnBnClickedExcel2)
	ON_WM_CLOSE()
	ON_COMMAND(ID_CON_LTE_CELL_DEL, &CCON_LTE_Cell::OnConLteCellDel)
	ON_NOTIFY(NM_RCLICK, IDC_LIST3, &CCON_LTE_Cell::OnNMRClickList3)
	ON_NOTIFY(NM_CLICK, IDC_LIST3, &CCON_LTE_Cell::OnNMClickList3)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST3, &CCON_LTE_Cell::OnLvnColumnclickList3)
END_MESSAGE_MAP()
...
#ifdef _DEBUG
void CCON_LTE_Cell::AssertValid() const
{
	CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CCON_LTE_Cell::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif
...
// CCON_LTE_Cell 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::OnInitialUpdate()
{
	CFormView::OnInitialUpdate();

	// TODO: 在此添加专用代码和/或调用基类
	//窗体名称
	v_sFrame_Name="LTE Cell 配置";										//窗体名称
	...
	m_cList_Station.InsertColumn(1,"RNC名称",LVCFMT_LEFT,100);			//
	m_cList_Station.InsertColumn(2,"NodeB",LVCFMT_LEFT,200);			//
	m_cList_Station.InsertColumn(3,"小区",LVCFMT_LEFT,200);				//
	m_cList_Station.InsertColumn(4,"中文名",LVCFMT_LEFT,200);			//
	m_cList_Station.InsertColumn(5,"LAC",LVCFMT_LEFT,80);				//
	...
	m_cList_Station.InsertColumn(15,"更新日期",LVCFMT_CENTER,100);		//
	...
	m_cKey_Select.AddString("E_NodeB");
...
//------------------------------------------------------------------------------------------------------			
BOOL CCON_LTE_Cell::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 在此添加专用代码和/或调用基类
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：站址导出
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::OnBnClickedExcel2()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
...
//	函数功能：站址导入
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::OnBnClickedElement()
{
	// TODO: 在此添加控件通知处理程序代码
	CCON_LTE_Cell_Input	v_cDlg;											//实例化
...
//	函数功能：列表点击响应
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::OnNMClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：列表右击响应
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::OnNMRClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	CMenu	aMenu;														//菜单
	...
	aMenu.LoadMenu(IDR_LTE_CELL);									//装载：IDR_GSM_BTS菜单
...
//	函数功能：删除小区
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::OnConLteCellDel()
{
	// TODO: 在此添加命令处理程序代码
	int		v_iItem,vi;													//临时变量
	...
	if(MessageBox("删除选中的 "+v_sTemp+" 个 LTE Cell， 删除确认 ? ","数据删除确认",MB_OKCANCEL|MB_ICONQUESTION)==IDOK)	//删除确认？？？
	{
		//对所有选中的Item进行操作
		while ((v_iItem = m_cList_Station.GetNextItem(-1, LVNI_SELECTED)) != -1)//条目选中？？？
		{
			v_sCell=m_cList_Station.GetItemText(v_iItem,3);				//获得：v_sCell [数据表唯一标识，用于修改数据表]

			//删除：Cell
			v_sSql = "delete from LTE_Cell where zCell='"+v_sCell+"'";	//SQL删除
...
//	函数功能：列表窗排序 [站址]
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::OnLvnColumnclickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：查询
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::OnBnClickedQuary()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CCON_LTE_Cell::My_Thread_Inoput(LPVOID lparam)
{
	CCON_LTE_Cell *  lp_this = NULL ;
	lp_this = (CCON_LTE_Cell *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 1:
...
//	函数功能：查询站址信息
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell::My_Query_Station()
{
	int				v_iID=0;											//临时变量
	...
	v_sSql += "SELECT LTE_Cell.*, LTE_NODEB.zRNC FROM LTE_Cell \
			  left JOIN LTE_NODEB ON LTE_Cell.zNODEB = LTE_NODEB.zNODEB where zRNC like '%dalian_%'"; 
	...
	if (!v_sLog_Key.IsEmpty())											//查询KEY处理
	{
		...
		case 1:															//NodeB
			...
			break;
		case 2:															//中文名
			v_sSql+=" and LTE_Cell.zCell_Name like '%"+v_sLog_Key+"%'";
			break;
		case 3:															//所属机房/资源点
...
//	函数功能：装载站址信息
//------------------------------------------------------------------------------------------------------			
void CCON_LTE_Cell::My_LoadData_Station_Data(CString v_sSql)
{
	...
	_variant_t	Var;
	CString			v_sStation,v_sElementName,v_sNodeB_Name,v_sCell,v_sCell_Name,v_sLac,v_sArea;	//临时变量
	CString			v_sOnline,v_sDate,v_sLongitude,v_sLatitude,v_sType;	//临时变量
	...
	try
	{
		...
		while(!v_pRecordset->adoEOF)
		{
			...
			if (Var.vt != VT_NULL)
				...
				v_sElementName="";
			Var=v_pRecordset->GetCollect((_variant_t)"zNODEB");			//NodeB名称
			if (Var.vt != VT_NULL)
			...
			if (Var.vt != VT_NULL)
				...
				v_sLac="";
			Var=v_pRecordset->GetCollect((_variant_t)"zRegion");		//区域
			if (Var.vt != VT_NULL)
				v_sArea=Var.bstrVal;
			else
				v_sArea="";
			...
			m_cList_Station.SetItemText(v_iItem,1,v_sElementName);		//网元名称
			m_cList_Station.SetItemText(v_iItem,2,v_sNodeB_Name);		//NodeB
			m_cList_Station.SetItemText(v_iItem,3,v_sCell);				//小区号
			m_cList_Station.SetItemText(v_iItem,4,v_sCell_Name);		//中文名
			m_cList_Station.SetItemText(v_iItem,5,v_sLac);				//LAC
			m_cList_Station.SetItemText(v_iItem,6,v_sArea);				//区域
			...
			m_cList_Station.SetItemText(v_iItem,15,v_sDate);			//更新日期
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CCON_LTE_Cell::My_ExecuteSQL(CString v_sSql)
{
