...
//mInt_Double		[通用容器：CInt_Double]
typedef	map<int,double> mInt_Double;									//map容器：mInt_Double
...
//mInt_Color		[通用容器：CInt_Color]
typedef	map<int,COLORREF> mInt_Color;									//map容器：mInt_Color
...
#include "MySplitter.h"													//屏幕窗体调整
#include "MyFormula.h"													//公式计算
#include "MyMap_Baidu.h"												//百度地图经纬度计算
...
struct  PM_Time_Struct
{  
	CString			v_sRegion;											//营业区
	CString			v_sOnline;											//状态
	map_PM_Time		v_mTime_PM_Data;									//容器：粒度-性能数据
};  
typedef map<CString,PM_Time_Struct>			map_PM_Cell;				//容器：性能_Cell

//LTE Cell_ID结构-------------------------------------------------------------------------------------------
struct  Cell_ID_Struct
{  
	CString			v_sCell;											//小区名称
	CString			v_sCell_Name;										//小区中文名
	CString			v_sRegion;											//营业区
	CString			v_sOnline;											//状态
}; 
typedef map<int,Cell_ID_Struct>				map_Cell_ID;				//容器：Cell_ID
...
struct  PM_Time_P_Struct
{  
	CString			v_sRegion;											//营业区
	CString			v_sOnline;											//状态
	map_PM_Time_P	v_mTime_PM_Data;									//容器：粒度-性能数据
};  
typedef map<CString,PM_Time_P_Struct>		map_PM_Cell_P;				//容器：性能_Cell_P
...
//低流量小区查询-----------------------------------------------------------------------------------
struct Alarm_LL_Cell
{
	bool			v_bFlag;											//有效标志
	COleDateTime	v_dDate;											//日期
	int				v_iHour;											//小时
	int				v_iCal;												//小于大于等于(0\1\2)
	double			v_dValues;											//数值
};

struct Alarm_LL_PM_DATA
{
	bool			v_bFlag;											//有效标志
	CString			v_sType;											//类型
	CString			v_sCell_ID;											//小区ID
	CString			v_sRegion;											//营业区
	CString			v_sOnline;											//状态
	double			v_dD_Values[5];										//下行粒度：数值
	double			v_dU_Values[5];										//上行粒度：数值
};
typedef map<CString,Alarm_LL_PM_DATA>		map_Alarm_LL_PM_DATA;		//容器
