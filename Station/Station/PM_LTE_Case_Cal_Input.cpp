// PM_LTE_Case_Cal_Input.cpp : 实现文件
//

#include "stdafx.h"
#include "Station.h"
#include "PM_LTE_Case_Cal_Input.h"


// CPM_LTE_Case_Cal_Input 对话框

IMPLEMENT_DYNAMIC(CPM_LTE_Case_Cal_Input, CDialog)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CPM_LTE_Case_Cal_Input::CPM_LTE_Case_Cal_Input(CWnd* pParent /*=NULL*/)
	: CDialog(CPM_LTE_Case_Cal_Input::IDD, pParent)
{
...
//	函数功能：析构函数
//------------------------------------------------------------------------------------------------------
CPM_LTE_Case_Cal_Input::~CPM_LTE_Case_Cal_Input()
{
}
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal_Input::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CPM_LTE_Case_Cal_Input, CDialog)
	ON_WM_SIZE()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BUTTON4, &CPM_LTE_Case_Cal_Input::OnBnClickedButton4)
	ON_BN_CLICKED(IDC_BUTTON1, &CPM_LTE_Case_Cal_Input::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON6, &CPM_LTE_Case_Cal_Input::OnBnClickedButton6)
END_MESSAGE_MAP()


// CPM_LTE_Case_Cal_Input 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
BOOL CPM_LTE_Case_Cal_Input::OnInitDialog()
{
	...
	m_cList.InsertColumn(4,"计算公式",LVCFMT_LEFT,400);					//
	m_cList.InsertColumn(5,"创建日期",LVCFMT_CENTER,120);				//
...
// 消息预处理：把回车键变换为TAB
//------------------------------------------------------------------------------------------------------			
BOOL CPM_LTE_Case_Cal_Input::PreTranslateMessage(MSG* pMsg)
{
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal_Input::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：关闭窗体
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal_Input::OnClose()
{
...
//	函数功能：异常导出
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal_Input::OnBnClickedButton6()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
	MyExcel.Show_Name(&m_cList_Bug,"导入异常_自定义计算性能",m_cFile_Open.GetCheck());		//导出Excel
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：获取资管数据CSV文件信息
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal_Input::OnBnClickedButton4()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：把列表窗中的数据保存到数据表
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal_Input::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CPM_LTE_Case_Cal_Input::My_Thread_Inoput(LPVOID lparam)
{
	CPM_LTE_Case_Cal_Input *  lp_this = NULL ;
	lp_this = (CPM_LTE_Case_Cal_Input *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal_Input::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 0:
...
//	函数功能：获取CSV文件信息
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal_Input::My_File()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	if(filedialog->DoModal()==IDOK)										//打开选择文件对话窗
	{
		...
		v_mMap.insert(pair<CString,int>("计算公式",4));					//4：计算公式
		v_mMap.insert(pair<CString,int>("创建日期",5));					//5：创建日期

		if (!Get_CSV_Cell(v_sFilename,&v_mMap,true))					//获取：CSV文件信息
			MessageBox("无有效的自定义计算性能文件，请重新选择 ！！！");//窗体提示	
	}
	delete(filedialog);
	v_mMap.clear();														//清除容器
}
...
//----------------------------------------------------------------------------------------------------
bool CPM_LTE_Case_Cal_Input::Get_CSV_Cell(CString v_sFile,mString_Int *v_mHead,bool v_bHead)
{
	...
	while(v_fRead.ReadString(v_sStr))									//数据读取：一行
	{
		...
		if (!v_sAccount.IsEmpty() && !v_sName.IsEmpty() && (v_sAccount==theApp.a_sUser_Account))	//条件：数据有效？？？
		{
			v_pRepeat=v_mRepeat.find(v_sAccount+v_sName);				//存在？？？
			if (v_pRepeat == v_mRepeat.end())							//不存在
			{
				v_mRepeat.insert(pair<CString,int>(v_sAccount+v_sName,0));	//容器：增加
			...
			{
				My_ShowList_Bug("查询模板导入异常","异常：模板重复 【模板名称："+v_sName+"】");		//显示：异常信息
			}
		}
		v_mData.clear();												//容器：清除
	}
...
//	函数功能：性能数据导入
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal_Input::My_Input_Cell()
{
	int		v_iLen,vi;													//临时变量
	CString	v_sSql,v_sTemp,v_sNew;										//临时变量
	CString	v_sAccount,v_sName,v_sFormula,v_sType;						//临时变量
	int		v_iSum=0;													//
	int		v_iSum_Sql=0;												//
	...
	if(MessageBox("计算性能数据 导入确认 ? ","数据导入确认",MB_OKCANCEL|MB_ICONQUESTION)!=IDOK)	//导入确认？？？
		return;															//返回：退出
	...
	if (v_iLen<=0)
	{
		MessageBox("没有可以导入的计算性能数据 ！！！");				//返回：无数据
		return;
	}
	...
	for (vi=0;vi<v_iLen;vi++)
	{
		...
		v_sAccount.Replace("（","(");									//规范化
		v_sAccount.Replace("）",")");									//规范化
		v_sName=m_cList.GetItemText(vi,2);								//模板名称
		v_sType=m_cList.GetItemText(vi,3);								//类型
		v_sFormula=m_cList.GetItemText(vi,4);							//计算公式

		v_iSum++;														//总数：性能
		v_pIter=v_mPM.find(v_sAccount+v_sName);							//查找：v_sAccount+v_sName
		if (v_pIter == v_mPM.end())										//没找到？？？
		{																//新增
			v_sNew="Insert into CON_PM_LTE_Case_Cal(zAccount,zName,zType,zPm_Formula,zDate) values ('"
					+v_sAccount+"','"+v_sName+"','"+v_sType+"','"+v_sFormula+"','"+v_sCurrentTime+"')";		//SQL
			m_cList.SetItemBkColor(vi,RGB(0,255,0));					//设置：背景色 [绿色]
		}
		else
		{
			v_sNew="Update CON_PM_LTE_Case_Cal set zType='"+v_sType+"',zPm_Formula='"+v_sFormula
					+"' where zAccount='"+v_sAccount+"' and zName='"+v_sName+"'";		//SQL
	...
	//日志
	v_sSql="Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"+v_sCurrentTime+"','自定义计算性能 数据导入','[用户："+theApp.a_sUser_Name+"]')";//SQL
	My_ExecuteSQL(v_sSql);												//数据库执行

	v_sTemp.Format("%d",v_iSum);										//总数：行
	m_cSum.SetWindowText(v_sTemp);										//总数：行
	v_sTemp.Format("模板总数：%d",v_iSum);
	MessageBox("自定义计算性能数据导入完成："+v_sTemp);					//返回：无数据
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：装载信息【性能计算】
//------------------------------------------------------------------------------------------------------			
void CPM_LTE_Case_Cal_Input::My_Load_PM(mString_Int *v_mPM)
{
	CString			v_sSql;												//临时变量
	...
	try
	{
		...
		v_sSql += "SELECT zAccount,zName FROM CON_PM_LTE_Case_Cal";
		...
		while(!v_pRecordset->adoEOF)
		{
			...
			v_mPM->insert(pair<CString,int>(v_sAccount+v_sName,0));		//容器：增加
...
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal_Input::My_ShowList_Bug(CString v_sBug_Type,CString v_sBug)
{
	int		v_iIndex;													//临时变量
	CString v_sTemp;													//临时变量
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CPM_LTE_Case_Cal_Input::My_ExecuteSQL(CString v_sSql)
{
