...
// CCON_NodeB_Input 对话框

IMPLEMENT_DYNAMIC(CCON_NodeB_Input, CDialog)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CCON_NodeB_Input::CCON_NodeB_Input(CWnd* pParent /*=NULL*/)
	: CDialog(CCON_NodeB_Input::IDD, pParent)
{
...
//	函数功能：析构函数
//------------------------------------------------------------------------------------------------------
CCON_NodeB_Input::~CCON_NodeB_Input()
{
}
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CCON_NodeB_Input::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CCON_NodeB_Input, CDialog)
	ON_WM_SIZE()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BUTTON4, &CCON_NodeB_Input::OnBnClickedButton4)
	ON_BN_CLICKED(IDC_BUTTON1, &CCON_NodeB_Input::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON6, &CCON_NodeB_Input::OnBnClickedButton6)
END_MESSAGE_MAP()


// CCON_NodeB_Input 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
BOOL CCON_NodeB_Input::OnInitDialog()
{
	...
	m_cList.InsertColumn(11,"入网日期",LVCFMT_CENTER,120);				//
	...
	m_SplitterPane_Main.Create(											//垂直线
		...
		IDC_CON_NodeB_Input,	// this ID is used for saving/restoring splitter position and it must be unique !
...
// 消息预处理：把回车键变换为TAB
//------------------------------------------------------------------------------------------------------			
BOOL CCON_NodeB_Input::PreTranslateMessage(MSG* pMsg)
{
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CCON_NodeB_Input::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：异常导出
//------------------------------------------------------------------------------------------------------
void CCON_NodeB_Input::OnBnClickedButton6()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
	MyExcel.Show_Name(&m_cList_Bug,"导入异常_TD_NodeB",m_cFile_Open.GetCheck());		//导出Excel
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：获取资管数据CSV文件信息
//------------------------------------------------------------------------------------------------------
void CCON_NodeB_Input::OnBnClickedButton4()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：把列表窗中的数据保存到数据表
//------------------------------------------------------------------------------------------------------
void CCON_NodeB_Input::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动	[静态门限计算]
//------------------------------------------------------------------------------------------------------
UINT CCON_NodeB_Input::My_Thread_Inoput(LPVOID lparam)
{
	CCON_NodeB_Input *  lp_this = NULL ;
	lp_this = (CCON_NodeB_Input *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CCON_NodeB_Input::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 0:
...
void CCON_NodeB_Input::My_File()
{
	...
	if(filedialog->DoModal()==IDOK)										//打开选择文件对话窗
	{
		...
		v_mMap.insert(pair<CString,int>("所属RNC",1));					//1：RNC
		v_mMap.insert(pair<CString,int>("网管中网元名称",2));			//2：NodeB
		v_mMap.insert(pair<CString,int>("NODEB名称",3));				//3：中文名
		v_mMap.insert(pair<CString,int>("设备型号",4));					//4：硬件
		v_mMap.insert(pair<CString,int>("IP",5));						//5：IP
		v_mMap.insert(pair<CString,int>("软件版本",6));					//6：软件版本
		v_mMap.insert(pair<CString,int>("所属区县",7));					//7：区域
		v_mMap.insert(pair<CString,int>("所属机房/资源点",8));			//8：所属机房/资源点
		v_mMap.insert(pair<CString,int>("覆盖类型",9));					//9：覆盖类型
		v_mMap.insert(pair<CString,int>("生命周期状态",10));			//10：状态
		v_mMap.insert(pair<CString,int>("入网时间",11));				//11：入网时间
		if (!Get_CSV_Cell(v_sFilename,&v_mMap,true))					//获取：CSV文件信息
			MessageBox("无有效的NodeB文件，请重新选择 ！！！");			//窗体提示	
	}
	delete(filedialog);
	v_mMap.clear();														//清除容器
}
...
//----------------------------------------------------------------------------------------------------
bool CCON_NodeB_Input::Get_CSV_Cell(CString v_sFile,mString_Int *v_mHead,bool v_bHead)
{
	...
	while(v_fRead.ReadString(v_sStr))									//数据读取：一行
	{
		...
		if (!v_sName.IsEmpty())											//条件：数据有效？？？
		{
			...
			{
				My_ShowList_Bug("NodeB异常","异常：NodeB名称重复 【NodeB名称："+v_sName+"】");		//显示：异常信息
			}
		}
		v_mData.clear();												//容器：清除
	}
...
//	函数功能：BTS数据导入
//------------------------------------------------------------------------------------------------------
void CCON_NodeB_Input::My_Input_BTS()
{
	int		vi,v_iLen;													//临时变量
	CString	v_sSql,v_sTemp;												//临时变量
	CString	v_sRegion,v_sRNC,v_sNodeB,v_sNodeB_Name,v_sIP,v_sType;		//临时变量
	...
	if(MessageBox("NodeB 数据 导入确认 ? ","数据导入确认",MB_OKCANCEL|MB_ICONQUESTION)!=IDOK)	//导入确认？？？
		return;															//返回：退出
	...
	if (v_iLen<=0)
	{
		MessageBox("没有可以导入的 NodeB 数据 ！！！");					//返回：无数据
		return;
	}
	...
	for (vi=0;vi<v_iLen;vi++)
	{
		...
		v_sNodeB_Name=m_cList.GetItemText(vi,3);						//中文名
		v_sHardware=m_cList.GetItemText(vi,4);							//硬件
		v_sIP=m_cList.GetItemText(vi,5);								//IP
		v_sSoftware=m_cList.GetItemText(vi,6);							//软件版本
		v_sRegion=m_cList.GetItemText(vi,7);							//区域
		v_sStation=m_cList.GetItemText(vi,8);							//所属机房/资源点
		v_sType=m_cList.GetItemText(vi,9);								//覆盖类型
		v_sOnline=m_cList.GetItemText(vi,10);							//状态
		...
		if (!v_sStation.IsEmpty())										//有效？？？
		{
			...
			{
				v_iSum++;												//总数：BTS
				v_pIter_BTS=v_mBTS.find(v_sNodeB);						//查找：v_sBTS
			...
			{
				My_ShowList_Bug("NodeB异常","异常：Station不存在；【NodeB名称："+v_sNodeB+"； 站址："+v_sStation+"  不存在】");	//显示：异常信息
				m_cList.SetItemBkColor(vi,RGB(255,0,0));				//设置：背景色 [红色]
			}
		}
		else
		{
			My_ShowList_Bug("NodeB异常","异常：Station名称异常；【NodeB名称："+v_sNodeB+"； 站址："+v_sStation+"】");	//显示：异常信息
			m_cList.SetItemBkColor(vi,RGB(255,0,0));					//设置：背景色 [红色]
		}
	...
	//日志
	v_sSql="Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"+v_sCurrentTime+"','TD NodeB 数据导入','[用户："+theApp.a_sUser_Name+"]')";//SQL
	My_ExecuteSQL(v_sSql);												//数据库执行

	v_sTemp.Format("%d",v_iSum);										//总数：行
	m_cSum.SetWindowText(v_sTemp);										//总数：行
	v_sTemp.Format("TD NodeB总数：%d",v_iSum);
	MessageBox("TD NodeB 数据导入完成："+v_sTemp);						//返回：无数据
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：装载信息【Station】
//------------------------------------------------------------------------------------------------------			
void CCON_NodeB_Input::My_Load_Station(mString_Int *v_mStation)
{
	CString			v_sSql;												//临时变量
...
//	函数功能：装载信息【BTS】
//------------------------------------------------------------------------------------------------------			
void CCON_NodeB_Input::My_Load_BTS(mString_Int *v_mBTS)
{
	CString			v_sSql;												//临时变量
	CString			v_sTemp,v_sName;									//临时变量
	...
	try
	{
		...
		//获取：NodeB
		v_sSql="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED ";
		v_sSql += "SELECT zNODEB FROM LTE_NODEB where zRNC like '%DAL-%'";
		v_pRecordset = v_cAdoConn.GetRecordSet((_bstr_t)v_sSql);		//执行SELETE语句										
		while(!v_pRecordset->adoEOF)
		{
...
//------------------------------------------------------------------------------------------------------
void CCON_NodeB_Input::My_ShowList_Bug(CString v_sBug_Type,CString v_sBug)
{
	int		v_iIndex;													//临时变量
	CString v_sTemp;													//临时变量
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CCON_NodeB_Input::My_ExecuteSQL(CString v_sSql)
{
