// PM_LTE_Region_Input.cpp : 实现文件
//

#include "stdafx.h"
#include "Station.h"
#include "PM_LTE_Region_Input.h"


// CPM_LTE_Region_Input 对话框

IMPLEMENT_DYNAMIC(CPM_LTE_Region_Input, CDialog)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CPM_LTE_Region_Input::CPM_LTE_Region_Input(CWnd* pParent /*=NULL*/)
	: CDialog(CPM_LTE_Region_Input::IDD, pParent)
{
...
//	函数功能：析构函数
//------------------------------------------------------------------------------------------------------
CPM_LTE_Region_Input::~CPM_LTE_Region_Input()
{
}
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region_Input::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CPM_LTE_Region_Input, CDialog)
	ON_WM_SIZE()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BUTTON4, &CPM_LTE_Region_Input::OnBnClickedButton4)
	ON_BN_CLICKED(IDC_BUTTON1, &CPM_LTE_Region_Input::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON6, &CPM_LTE_Region_Input::OnBnClickedButton6)
END_MESSAGE_MAP()


// CPM_LTE_Region_Input 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
BOOL CPM_LTE_Region_Input::OnInitDialog()
{
	...
	m_SplitterPane_Main.Create(											//垂直线
		...
		IDC_PM_LTE_Region_Input,// this ID is used for saving/restoring splitter position and it must be unique !
...
// 消息预处理：把回车键变换为TAB
//------------------------------------------------------------------------------------------------------			
BOOL CPM_LTE_Region_Input::PreTranslateMessage(MSG* pMsg)
{
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region_Input::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：关闭窗体
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region_Input::OnClose()
{
...
//	函数功能：异常导出
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region_Input::OnBnClickedButton6()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
	MyExcel.Show_Name(&m_cList_Bug,"导入异常_性能_区域",m_cFile_Open.GetCheck());		//导出Excel
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：获取资管数据CSV文件信息
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region_Input::OnBnClickedButton4()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：把列表窗中的数据保存到数据表
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region_Input::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CPM_LTE_Region_Input::My_Thread_Inoput(LPVOID lparam)
{
	CPM_LTE_Region_Input *  lp_this = NULL ;
	lp_this = (CPM_LTE_Region_Input *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region_Input::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 0:
...
//	函数功能：获取CSV文件信息
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region_Input::My_File()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	if(filedialog->DoModal()==IDOK)										//打开选择文件对话窗
	{
		...
		if (!Get_CSV_Cell(v_sFilename,&v_mMap,true))					//获取：CSV文件信息
			MessageBox("无有效的性能_区域文件，请重新选择 ！！！");		//窗体提示	
	}
	delete(filedialog);
	v_mMap.clear();														//清除容器
}
...
//----------------------------------------------------------------------------------------------------
bool CPM_LTE_Region_Input::Get_CSV_Cell(CString v_sFile,mString_Int *v_mHead,bool v_bHead)
{
	...
	while(v_fRead.ReadString(v_sStr))									//数据读取：一行
	{
		...
		if (!v_sName.IsEmpty())											//条件：数据有效？？？
		{
			...
			{
				My_ShowList_Bug("性能区域异常","异常：性能名称重复 【性能名称："+v_sName+"】");		//显示：异常信息
			}
		}
		v_mData.clear();												//容器：清除
	}
...
//	函数功能：性能数据导入
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region_Input::My_Input_Cell()
{
	int		v_iLen,vi;													//临时变量
	...
	for (vi=0;vi<v_iLen;vi++)
	{
		...
		{																//新增
			v_sNew="Insert into CON_Pm_LTE_Region(zPm,zPm_Name,zPm_Formula,zSave_Flag,zAlarm_Flag,\
		...
		{
			v_sNew="Update CON_Pm_LTE_Region set zPm_Name='"+v_sPM_Name+"',zPm_Formula='"+v_sFormula
	...
	//日志
	v_sSql="Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"+v_sCurrentTime+"','性能区域 数据导入','[用户："+theApp.a_sUser_Name+"]')";//SQL
	My_ExecuteSQL(v_sSql);												//数据库执行

	v_sTemp.Format("%d",v_iSum);										//总数：行
	m_cSum.SetWindowText(v_sTemp);										//总数：行
	v_sTemp.Format("LTE Cell总数：%d",v_iSum);
	MessageBox("性能区域数据导入完成："+v_sTemp);						//返回：无数据
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：装载信息【性能计算】
//------------------------------------------------------------------------------------------------------			
void CPM_LTE_Region_Input::My_Load_PM(mString_Int *v_mPM)
{
	CString			v_sSql;												//临时变量
	...
	try
	{
		...
		v_sSql += "SELECT zPM FROM CON_Pm_LTE_Region";
...
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region_Input::My_ShowList_Bug(CString v_sBug_Type,CString v_sBug)
{
	int		v_iIndex;													//临时变量
	CString v_sTemp;													//临时变量
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CPM_LTE_Region_Input::My_ExecuteSQL(CString v_sSql)
{
