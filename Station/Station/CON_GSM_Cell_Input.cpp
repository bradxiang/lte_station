...
// CCON_GSM_Cell_Input 对话框

IMPLEMENT_DYNAMIC(CCON_GSM_Cell_Input, CDialog)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CCON_GSM_Cell_Input::CCON_GSM_Cell_Input(CWnd* pParent /*=NULL*/)
	: CDialog(CCON_GSM_Cell_Input::IDD, pParent)
{
...
//	函数功能：析构函数
//------------------------------------------------------------------------------------------------------
CCON_GSM_Cell_Input::~CCON_GSM_Cell_Input()
{
}
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CCON_GSM_Cell_Input::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CCON_GSM_Cell_Input, CDialog)
	ON_WM_SIZE()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BUTTON4, &CCON_GSM_Cell_Input::OnBnClickedButton4)
	ON_BN_CLICKED(IDC_BUTTON1, &CCON_GSM_Cell_Input::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON6, &CCON_GSM_Cell_Input::OnBnClickedButton6)
END_MESSAGE_MAP()


// CCON_GSM_Cell_Input 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
BOOL CCON_GSM_Cell_Input::OnInitDialog()
{
	...
	m_cList.InsertColumn(1,"Cell",LVCFMT_LEFT,80);						//
	m_cList.InsertColumn(2,"BTS",LVCFMT_LEFT,80);						//
	m_cList.InsertColumn(3,"中文名",LVCFMT_LEFT,200);					//
	m_cList.InsertColumn(4,"LAC",LVCFMT_LEFT,100);						//
	m_cList.InsertColumn(5,"区域",LVCFMT_LEFT,80);						//
	m_cList.InsertColumn(6,"所属机房/资源点",LVCFMT_LEFT,260);			//
	m_cList.InsertColumn(7,"覆盖类型",LVCFMT_LEFT,70);					//
	m_cList.InsertColumn(8,"状态",LVCFMT_CENTER,60);					//
	m_cList.InsertColumn(9,"入网日期",LVCFMT_CENTER,120);				//
	...
	m_SplitterPane_Main.Create(											//垂直线
		...
		IDC_CON_GSM_Cell_Input,	// this ID is used for saving/restoring splitter position and it must be unique !
...
// 消息预处理：把回车键变换为TAB
//------------------------------------------------------------------------------------------------------			
BOOL CCON_GSM_Cell_Input::PreTranslateMessage(MSG* pMsg)
{
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CCON_GSM_Cell_Input::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：关闭窗体
//------------------------------------------------------------------------------------------------------
void CCON_GSM_Cell_Input::OnClose()
{
...
//	函数功能：异常导出
//------------------------------------------------------------------------------------------------------
void CCON_GSM_Cell_Input::OnBnClickedButton6()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
	MyExcel.Show_Name(&m_cList_Bug,"导入异常_GSM_Cell",m_cFile_Open.GetCheck());		//导出Excel
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：获取资管数据CSV文件信息
//------------------------------------------------------------------------------------------------------
void CCON_GSM_Cell_Input::OnBnClickedButton4()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：把列表窗中的数据保存到数据表
//------------------------------------------------------------------------------------------------------
void CCON_GSM_Cell_Input::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动	[静态门限计算]
//------------------------------------------------------------------------------------------------------
UINT CCON_GSM_Cell_Input::My_Thread_Inoput(LPVOID lparam)
{
	CCON_GSM_Cell_Input *  lp_this = NULL ;
	lp_this = (CCON_GSM_Cell_Input *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CCON_GSM_Cell_Input::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 0:
...
//	函数功能：获取CSV文件信息
//------------------------------------------------------------------------------------------------------
void CCON_GSM_Cell_Input::My_File()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	if(filedialog->DoModal()==IDOK)										//打开选择文件对话窗
	{
		...
		v_mMap.insert(pair<CString,int>("网管中网元名称",1));			//1：Cell
		v_mMap.insert(pair<CString,int>("所属BTS",2));					//2：BTS
		v_mMap.insert(pair<CString,int>("CELL名称",3));					//3：中文名
		v_mMap.insert(pair<CString,int>("位置区码LAC",4));				//4：LAC
		v_mMap.insert(pair<CString,int>("区/县名称",5));				//5：区域
		v_mMap.insert(pair<CString,int>("所属机房/资源点",6));			//6：所属机房/资源点
		v_mMap.insert(pair<CString,int>("覆盖类型",7));					//7：覆盖类型
		v_mMap.insert(pair<CString,int>("生命周期状态",8));				//8：状态
		v_mMap.insert(pair<CString,int>("入网时间",9));					//9：入网日期
...
//----------------------------------------------------------------------------------------------------
bool CCON_GSM_Cell_Input::Get_CSV_Cell(CString v_sFile,mString_Int *v_mHead,bool v_bHead)
{
	...
	while(v_fRead.ReadString(v_sStr))									//数据读取：一行
	{
		...
		{
			v_pRepeat=v_mRepeat.find(v_sName);							//Cell存在？？？
			if (v_pRepeat == v_mRepeat.end())							//Cell：不存在
			{
...
//	函数功能：Cell数据导入
//------------------------------------------------------------------------------------------------------
void CCON_GSM_Cell_Input::My_Input_Cell()
{
	int		v_iLen,vi;													//临时变量
	CString	v_sSql,v_sTemp;												//临时变量
	CString	v_sCell,v_sType,v_sRegion,v_sBTS,v_sBTS_Name,v_sLAC;		//临时变量
	...
	for (vi=0;vi<v_iLen;vi++)
	{
		...
		v_sBTS=m_cList.GetItemText(vi,2);								//BTS
		v_sBTS_Name=m_cList.GetItemText(vi,3);							//中文名
		v_sLAC=m_cList.GetItemText(vi,4);								//LAC
		v_sRegion=m_cList.GetItemText(vi,5);							//区域
		v_sStation=m_cList.GetItemText(vi,6);							//所属机房/资源点
		v_sType=m_cList.GetItemText(vi,7);								//基站类型
		v_sOnline=m_cList.GetItemText(vi,8);							//状态

		if (m_cOnline.GetCheck() && v_sOnline!="在网")					//在网有效？？？
			continue;													//下一条

		if (v_sCell.GetLength()==7)										//有效？？？
		{
			...
			if (v_pIter_BTS != v_mBTS.end())							//找到？？？
			{
				...
				{														//新增
					v_sNew="Insert into GSM_Cell(zCell,zBTS,zStation,zType,zLAC,zRegion,zOnline,zDate) values ('"
							+v_sCell+"','"+v_sBTS+"','"+v_sStation+"','"+v_sType+"','"+v_sLAC+"','"+v_sRegion+"','"
							+v_sOnline+"','"+v_sCurrentTime+"')";		//SQL
					m_cList.SetItemBkColor(vi,RGB(0,255,0));			//设置：背景色 [绿色]
				}
				else
				{
					v_sNew="Update GSM_Cell set zBTS='"+v_sBTS+"',zStation='"+v_sStation+"',zType='"+v_sType
						+"',zLAC='"+v_sLAC+"',zRegion='"+v_sRegion+"',zOnline='"+v_sOnline+"',zDate='"+v_sCurrentTime
			...
			{
				My_ShowList_Bug("Cell异常","异常：BTS不存在；【Cell名称："+v_sCell+"； 所属BTS："+v_sBTS+"  不存在"+"；中文名："+v_sBTS_Name+"】");	//显示：异常信息
				m_cList.SetItemBkColor(vi,RGB(255,0,0));				//设置：背景色 [红色]
			}
		}
		else
		{
			My_ShowList_Bug("Cell异常","异常：Cell名称异常；【Cell名称："+v_sCell+"； 所属BTS："+v_sBTS+"；中文名："+v_sBTS_Name+"】");	//显示：异常信息
			m_cList.SetItemBkColor(vi,RGB(255,0,0));					//设置：背景色 [红色]
		}
	...
	//日志
	v_sSql="Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"+v_sCurrentTime+"','GSM Cell 数据导入','[用户："+theApp.a_sUser_Name+"]')";//SQL
	My_ExecuteSQL(v_sSql);												//数据库执行

	v_sTemp.Format("%d",v_iSum);										//总数：行
	m_cSum.SetWindowText(v_sTemp);										//总数：行
	v_sTemp.Format("GSM Cell总数：%d",v_iSum);
	MessageBox("GSM Cell数据导入完成："+v_sTemp);						//返回：无数据
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：装载信息【Cell】
//------------------------------------------------------------------------------------------------------			
void CCON_GSM_Cell_Input::My_Load_Cell(mString_Int *v_mCell)
{
	CString			v_sSql;												//临时变量
	...
	try
	{
		...
		v_sSql += "SELECT zCell FROM GSM_Cell";
...
//	函数功能：装载信息【BTS】
//------------------------------------------------------------------------------------------------------			
void CCON_GSM_Cell_Input::My_Load_BTS(mString_Int *v_mBTS)
{
	CString			v_sSql;												//临时变量
	CString			v_sTemp,v_sName;									//临时变量
...
//------------------------------------------------------------------------------------------------------
void CCON_GSM_Cell_Input::My_ShowList_Bug(CString v_sBug_Type,CString v_sBug)
{
	int		v_iIndex;													//临时变量
	CString v_sTemp;													//临时变量
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CCON_GSM_Cell_Input::My_ExecuteSQL(CString v_sSql)
{
