// PM_LTE_Case_Cell_Input.cpp : 实现文件
//
...
// CPM_LTE_Case_Cell_Input 消息处理程序
...
BOOL CPM_LTE_Case_Cell_Input::OnInitDialog()
{
	...
	m_cList.InsertColumn(3,"小区",LVCFMT_LEFT,300);						//
...
void CPM_LTE_Case_Cell_Input::OnBnClickedButton6()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
	MyExcel.Show_Name(&m_cList_Bug,"导入异常_性能查询模板_小区",m_cFile_Open.GetCheck());		//导出Excel
}
...
void CPM_LTE_Case_Cell_Input::My_File()
{
	...
	if(filedialog->DoModal()==IDOK)										//打开选择文件对话窗
	{
		...
		v_mMap.insert(pair<CString,int>("小区",3));						//3：小区
		v_mMap.insert(pair<CString,int>("创建日期",4));					//4：创建日期

		if (!Get_CSV_Cell(v_sFilename,&v_mMap,true))					//获取：CSV文件信息
			MessageBox("无有效的性能查询模板_小区文件，请重新选择 ！！！");		//窗体提示	
	}
	delete(filedialog);
	v_mMap.clear();														//清除容器
}
...
bool CPM_LTE_Case_Cell_Input::Get_CSV_Cell(CString v_sFile,mString_Int *v_mHead,bool v_bHead)
{
	...
	CString		v_sAccount,v_sName,v_sCell;								//临时变量
	...
	while(v_fRead.ReadString(v_sStr))									//数据读取：一行
	{
		...
		{
			v_pRepeat=v_mRepeat.find(v_sAccount+v_sName+v_sCell);		//存在？？？
			if (v_pRepeat == v_mRepeat.end())							//不存在
			{
				v_mRepeat.insert(pair<CString,int>(v_sAccount+v_sName+v_sCell,0));	//容器：增加
			...
			{
				My_ShowList_Bug("查询模板导入异常","异常：小区重复 【模板名称："+v_sName+"；小区："+v_sCell+"】");		//显示：异常信息
			}
		}
		v_mData.clear();												//容器：清除
	}
...
void CPM_LTE_Case_Cell_Input::My_Input_Cell()
{
	...
	CString	v_sName,v_sCell,v_sAccount,v_sCurTime;						//临时变量
	int		v_iSum=0;													//
	int		v_iSum_Sql=0;												//
	...
	//第二部：获取相关数据 [模板]
	My_Load_PM(&v_mPM,&v_mCase);										//获取：信息 [放入：v_mPM]
	...
	for (vi=0;vi<v_iLen;vi++)
	{
		...
		v_sCell=m_cList.GetItemText(vi,3);								//小区
		v_sCell.Replace("（","(");										//规范化
		v_sCell.Replace("）",")");										//规范化

		v_iSum++;														//总数：小区
		v_pIter=v_mPM.find(v_sAccount+v_sName+v_sCell);					//查找：模板
	...
	MessageBox("查询模板数据导入完成："+v_sTemp);						//返回：无数据
}
...
{
	CString			v_sSql;												//临时变量
	CString			v_sTemp,v_sName,v_sCell,v_sAccount;					//临时变量
	...
	try
	{
		...
		v_sSql += "SELECT zAccount,zName,zCell FROM CON_Pm_LTE_Case_Cell_Data where zAccount='共享' or zAccount='"+
		...
		while(!v_pRecordset->adoEOF)
		{
			...
			v_sCell = v_cAdoConn.GetField_String("zCell");				//小区

			v_mPM->insert(pair<CString,int>(v_sAccount+v_sName+v_sCell,0));		//容器：增加
