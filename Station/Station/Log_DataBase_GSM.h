...
// CLog_DataBase_GSM 窗体视图

class CLog_DataBase_GSM : public CFormView
{
	DECLARE_DYNCREATE(CLog_DataBase_GSM)

protected:
	CLog_DataBase_GSM();           // 动态创建所使用的受保护的构造函数
	virtual ~CLog_DataBase_GSM();

public:
	enum { IDD = IDD_LOG_DATABASE_GSM };
#ifdef _DEBUG
...
public:
	...
	void		My_LoadSize(CString v_sTable,CString v_sAlarm_Name,CString v_sType,int v_iType);	//获取：基础表容量获取、显示
	void		My_LoadSize_Table(CString v_sTable,int v_iNum);			//获取：表容量信息
	void		My_LoadSize_Show(CString v_sTable,CString v_sFlag,int v_iType,int v_iSum);			//显示：容量信息添加到列表
