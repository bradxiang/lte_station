...
public:
	...
	void		My_Query_Region();										//区域核查
	void		My_LoadData_Station_Data(CString v_sSql);				//装载站址信息
	void		My_Show_Region(mStation_OutPut *v_mStation_OutPut);		//区域显示
	...
	typedef map<CString,Station_Long_Struct> mStation_Long;				//容器：mStation_Long
	CButton m_cRemote;
	CButton m_cStation;
	CButton m_cCell;
};
