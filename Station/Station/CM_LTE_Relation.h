...
protected:
	CCM_LTE_Relation();           // 动态创建所使用的受保护的构造函数
...
public:
	...
	void		My_LoadData_CM_Data(CString v_sTable);					//获取配置数据

	void		My_LoadData_Para();										//装载性能参数信息
	void		My_LoadData_Cell();										//装载小区信息
	void		My_Get_Cm_Field();										//获取配置参数(列显示)
	...
	CString			v_sOutput;											//文件输出

	//性能数据
	set<CString>	v_mTable_Name;										//集合：性能表名称
	mString_Int		v_mField_Main;										//选择的字段容器

	mString_String	v_mCell;											//小区容器[用于 E_NodeB_ID+Cell_ID 影射 小区名称]
};
