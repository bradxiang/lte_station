...
void CCON_E_NodeB_Input::OnBnClickedButton6()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
	MyExcel.Show_Name(&m_cList_Bug,"导入异常_LTE_E_NodeB",m_cFile_Open.GetCheck());		//导出Excel
}
...
void CCON_E_NodeB_Input::My_File()
{
	...
	if(filedialog->DoModal()==IDOK)										//打开选择文件对话窗
	{
		...
		v_mMap.insert(pair<CString,int>("网管中网元名称",2));			//2：E_NodeB
		v_mMap.insert(pair<CString,int>("E-NODEB名称",3));				//3：中文名
		v_mMap.insert(pair<CString,int>("设备型号",4));					//4：硬件
		v_mMap.insert(pair<CString,int>("X2 IP地址",5));				//5：IP
...
bool CCON_E_NodeB_Input::Get_CSV_Cell(CString v_sFile,mString_Int *v_mHead,bool v_bHead)
{
	...
	while(v_fRead.ReadString(v_sStr))									//数据读取：一行
	{
		...
		if (!v_sName.IsEmpty())											//条件：数据有效？？？
		{
			...
			{
				My_ShowList_Bug("E_NodeB异常","异常：E_NodeB称重复 【E_NodeB名称："+v_sName+"】");		//显示：异常信息
			}
		}
		v_mData.clear();												//容器：清除
	}
...
void CCON_E_NodeB_Input::My_Input_BTS()
{
	...
	if(MessageBox("E_NodeB 数据 导入确认 ? ","数据导入确认",MB_OKCANCEL|MB_ICONQUESTION)!=IDOK)	//导入确认？？？
		return;															//返回：退出
	...
	if (v_iLen<=0)
	{
		MessageBox("没有可以导入的 E_NodeB 数据 ！！！");				//返回：无数据
		return;
	}
	...
	for (vi=0;vi<v_iLen;vi++)
	{
		...
		v_sType=m_cList.GetItemText(vi,9);								//覆盖范围
		...
		if (!v_sStation.IsEmpty())										//有效？？？
		{
			...
			{
				My_ShowList_Bug("E_NodeB异常","异常：Station不存在；【E_NodeB名称："+v_sNodeB+"； 站址："+v_sStation+"  不存在】");	//显示：异常信息
				m_cList.SetItemBkColor(vi,RGB(255,0,0));				//设置：背景色 [红色]
			}
		}
		else
		{
			My_ShowList_Bug("E_NodeB异常","异常：Station名称异常；【E_NodeB名称："+v_sNodeB+"； 站址："+v_sStation+"】");	//显示：异常信息
			m_cList.SetItemBkColor(vi,RGB(255,0,0));					//设置：背景色 [红色]
		}
	...
	//日志
	v_sSql="Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"+v_sCurrentTime+"','LTE E_NodeB 数据导入','[用户："+theApp.a_sUser_Name+"]')";//SQL
	My_ExecuteSQL(v_sSql);												//数据库执行

	v_sTemp.Format("%d",v_iSum);										//总数：行
	m_cSum.SetWindowText(v_sTemp);										//总数：行
	v_sTemp.Format("LTE E_NodeB总数：%d",v_iSum);
	MessageBox("LTE E_NodeB 数据导入完成："+v_sTemp);					//返回：无数据
}
