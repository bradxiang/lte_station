...
void CQUA_Station::OnInitialUpdate()
{
	...
	m_cList_BTS.InsertColumn(0,"序号",LVCFMT_CENTER,60);				//
	m_cList_BTS.InsertColumn(1,"覆盖",LVCFMT_CENTER,50);				//
	m_cList_BTS.InsertColumn(2,"站址-基站",LVCFMT_LEFT,240);			//
	m_cList_BTS.InsertColumn(3,"站址-小区",LVCFMT_LEFT,240);			//
	m_cList_BTS.InsertColumn(4,"相同",LVCFMT_LEFT,60);					//
	m_cList_BTS.InsertColumn(5,"基站",LVCFMT_LEFT,240);					//
	m_cList_BTS.InsertColumn(6,"小区",LVCFMT_LEFT,240);					//
...
void CQUA_Station::OnBnClickedButton7()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
	MyExcel.Show(&m_cList_BTS);											//导出Excel
}
...
void CQUA_Station::OnNMClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (pNMListView->iItem>=0)											//判断：条目序号有效？？？
	{
		if (v_pIterator_Frame->second.v_iThread_Exit==0)
		{
			...
			My_Show_Tree(v_sRoom);										//显示：基站、小区信息
		}
	}
...
void CQUA_Station::My_LoadData_Station_Data(CString v_sSql)
{
	...
	try
	{
		...
		m_cList_BTS.DeleteAllItems();									//容器清除
...
{
	CString			v_sTemp,v_sRoom_BTS,v_sBTS,v_sCover;				//临时变量
	...
	for (v_pRoom=v_mTree_Room.begin();v_pRoom!=v_mTree_Room.end();v_pRoom++)	//遍历
	{
		if(v_pRoom->second.v_iType==0)									//本站？？？
		{
			//(1)本站：根目录
			v_bOK=true;													//置位
			v_sRoom_BTS=v_pRoom->first;									//机房：基站
			...
			while(v_pBTS!=v_pRoom->second.v_mTree_BTS.end())			//遍历
			{
				...
				if (v_pBTS->first.GetLength()==6)						//GSM ???
					v_sBTS=v_pBTS->first+"_"+v_pBTS->second.v_sBTS_Name;	//BTS+中文名
				else
					v_sBTS=v_pBTS->first;								//BTS
				v_sCover=v_pBTS->second.v_sCover;						//覆盖

				//(5)添加：小区信息
				v_pCell=v_pBTS->second.v_mTree_Cell.begin();			//头指针
				while(v_pCell!=v_pBTS->second.v_mTree_Cell.end())		//遍历
				{
					My_ShowList_BTS(v_sCover,v_sRoom_BTS,v_pCell->second.v_sStation,"相同",v_sBTS,v_pCell->first);	//列表显示
					v_pCell++;											//指针++
				}
				v_pBTS++;												//指针++
			}
		...
		for (v_pRoom=v_mTree_Room.begin();v_pRoom!=v_mTree_Room.end();v_pRoom++)	//遍历
		{
			if(v_pRoom->second.v_iType==1)								//拉远？？？
			{
				...
				for(v_pBTS=v_pRoom->second.v_mTree_BTS.begin();v_pBTS!=v_pRoom->second.v_mTree_BTS.end();v_pBTS++)	//遍历
				{
					...
					while(v_pCell!=v_pBTS->second.v_mTree_Cell.end())	//遍历
					{
						My_ShowList_BTS(v_sCover,v_sRoom_BTS,v_pCell->second.v_sStation,"",v_sBTS,v_pCell->first);	//列表显示
						v_pCell++;										//指针++
					}
				}
			}
	...
	for (v_pRoom=v_mTree_Room.begin();v_pRoom!=v_mTree_Room.end();v_pRoom++)	//遍历
	{
		...
		{
			//(1)本站：根目录
			v_sRoom_BTS=v_pRoom->first;									//树：增加根节点
			...
			while(v_pBTS!=v_pRoom->second.v_mTree_BTS.end())			//遍历
			{
				...
				while(v_pCell!=v_pBTS->second.v_mTree_Cell.end())		//遍历
				{
					v_sTemp="";
					if (v_bOK)
						v_sTemp=(v_sRoom==v_pCell->second.v_sStation)?"从站":"";	//
					My_ShowList_BTS(v_sCover,v_sRoom_BTS,v_pCell->second.v_sStation,v_sTemp,v_sBTS,v_pCell->first);	//列表显示
					v_pCell++;											//指针++
				}
				v_pBTS++;												//指针++
			}
...
void CQUA_Station::My_ShowList_BTS(CString v_s1,CString v_s2,CString v_s3,CString v_s4,CString v_s5,CString v_s6)
{
	...
	m_cList_BTS.SetItemText(v_iIndex,0," "+v_sTemp);					//序号
	...
	m_cList_BTS.SetRedraw(FALSE);										//不更新界面
	if (m_cList_BTS.GetItemCount()>=1000)								//如果纪录数大于1000
	{
		for(vi=0;vi<300;vi++)											//删除前300条
			m_cList_BTS.DeleteItem(0);									//删除第一条数据记录
		for(vi=0;vi<m_cList_BTS.GetItemCount();vi++)					//重新分配序号
		{
			v_sTemp.Format("%04d",vi+1);
			m_cList_BTS.SetItemText(vi,0," "+v_sTemp);
		}
	}
	m_cList_BTS.SetRedraw(true);										//更新界面
	m_cList_BTS.EnsureVisible(m_cList_BTS.GetItemCount()-1,false);		//显示最新一条记录
}
