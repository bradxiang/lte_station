...
//-------------------------------------------------------------------------------------------------
void CMyExcel::Show(CMyListCtrl *v_cList) 
{
	if (v_cList->GetStyle() & LVS_OWNERDATA)							//虚拟列表
		v_cList->CSV_OutPut_Virtual("",1);								//写入CSV，文件名称：默认；用Excel打开文件[虚拟列表]
	else																//常规列表
		CSV_OutPut(v_cList,"",1);										//写入CSV，文件名称：默认；用Excel打开文件
}
...
//-------------------------------------------------------------------------------------------------
void CMyExcel::Show_Name(CMyListCtrl *v_cList,CString v_sFile_Name,int v_bFile_Open) 
{
	if (v_cList->GetStyle() & LVS_OWNERDATA)							//虚拟列表
		v_cList->CSV_OutPut_Virtual(v_sFile_Name,v_bFile_Open);			//写入CSV，文件名称：默认；用Excel打开文件[虚拟列表]
	else																//常规列表
		CSV_OutPut(v_cList,v_sFile_Name,v_bFile_Open);					//写入CSV，文件名称：默认；用Excel打开文件
}

//-------------------------------------------------------------------------------------------------
// 导出Excel处理：写入CSV文件；【常规列表】
// 参数：v_cList：列表窗指针
//		 v_sFile_Name：输出的文件名称
//		 v_bFile_Open：文件打开标志；[1：打开；0：不打开；]
//-------------------------------------------------------------------------------------------------
void CMyExcel::CSV_OutPut(CMyListCtrl *v_cList,CString v_sFile_Name,int v_bFile_Open) 
{
	int		v_iNum1,v_iNum2,v_iNum3;									//临时变量
	...
	if (!finder.FindFile(v_sPath_Data))									//存在？
	{
		...
		{
			MessageBoxA(NULL,"异常："+v_sPath_Data+" 目录创建失败。", "提示", MB_OK);		//失败
			return;
		}
	}
...
//----------------------------------------------------------------------------------------------------
bool CMyExcel::Get_CSV(CString v_sFile,CMyListCtrl *v_cList,bool v_bHead)
{
