...
void CCON_TD_Cell::OnInitialUpdate()
{
	...
	m_cList_Station.InsertColumn(5,"区域",LVCFMT_LEFT,80);				//
	m_cList_Station.InsertColumn(6,"所属机房/资源点",LVCFMT_LEFT,260);	//
	m_cList_Station.InsertColumn(7,"覆盖类型",LVCFMT_LEFT,70);			//
	m_cList_Station.InsertColumn(8,"经度",LVCFMT_RIGHT,100);			//
	m_cList_Station.InsertColumn(9,"纬度",LVCFMT_RIGHT,100);			//
	m_cList_Station.InsertColumn(10,"天线方向",LVCFMT_RIGHT,100);		//
	m_cList_Station.InsertColumn(11,"天线高度",LVCFMT_RIGHT,100);		//
	m_cList_Station.InsertColumn(12,"天线倾角",LVCFMT_RIGHT,100);		//
	m_cList_Station.InsertColumn(13,"状态",LVCFMT_CENTER,60);			//
	m_cList_Station.InsertColumn(14,"更新日期",LVCFMT_CENTER,100);		//
...
void CCON_TD_Cell::OnNMRClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	aMenu.LoadMenu(IDR_TD_CELL);									//装载：IDR_GSM_BTS菜单
...
void CCON_TD_Cell::OnConTdCellDel()
{
	...
	if(MessageBox("删除选中的 "+v_sTemp+" 个 TD Cell， 删除确认 ? ","数据删除确认",MB_OKCANCEL|MB_ICONQUESTION)==IDOK)	//删除确认？？？
	{
		//对所有选中的Item进行操作
...
void CCON_TD_Cell::My_LoadData_Station_Data(CString v_sSql)
{
	...
	_variant_t	Var;
	CString			v_sStation,v_sElementName,v_sNodeB_Name,v_sCell,v_sLac,v_sArea;		//临时变量
	...
	try
	{
		...
		while(!v_pRecordset->adoEOF)
		{
			...
			m_cList_Station.SetItemText(v_iItem,5,v_sArea);				//区域
			m_cList_Station.SetItemText(v_iItem,6,v_sStation);			//所属机房/资源点
			m_cList_Station.SetItemText(v_iItem,7,v_sType);				//覆盖类型
			m_cList_Station.SetItemText(v_iItem,8,v_sLongitude);		//经度
			m_cList_Station.SetItemText(v_iItem,9,v_sLatitude);			//纬度
			v_sTemp.Format("%d",v_iAnt_Angle);							//
			m_cList_Station.SetItemText(v_iItem,10,v_sTemp);			//天线方向
			v_sTemp.Format("%d",v_iAnt_High);							//
			m_cList_Station.SetItemText(v_iItem,11,v_sTemp);			//天线高度
			v_sTemp.Format("%d",v_iAnt_Dip);							//
			m_cList_Station.SetItemText(v_iItem,12,v_sTemp);			//天线倾角
			m_cList_Station.SetItemText(v_iItem,13,v_sOnline);			//状态
			m_cList_Station.SetItemText(v_iItem,14,v_sDate);			//更新日期
