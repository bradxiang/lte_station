// PM_LTE_Case_Cell.cpp : 实现文件
//

#include "stdafx.h"
#include "Station.h"
#include "PM_LTE_Case_Cell.h"
#include "PM_LTE_Case_Cell_Input.h"


// CPM_LTE_Case_Cell

IMPLEMENT_DYNCREATE(CPM_LTE_Case_Cell, CFormView)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CPM_LTE_Case_Cell::CPM_LTE_Case_Cell()
	: CFormView(CPM_LTE_Case_Cell::IDD)
{
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CPM_LTE_Case_Cell, CFormView)
	ON_WM_DESTROY()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_EXCEL2, &CPM_LTE_Case_Cell::OnBnClickedExcel2)
	ON_WM_CLOSE()
	ON_NOTIFY(NM_CLICK, IDC_LIST3, &CPM_LTE_Case_Cell::OnNMClickList3)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST3, &CPM_LTE_Case_Cell::OnLvnColumnclickList3)
	ON_BN_CLICKED(IDC_QUARY2, &CPM_LTE_Case_Cell::OnBnClickedQuary2)
	ON_NOTIFY(NM_RCLICK, IDC_LIST3, &CPM_LTE_Case_Cell::OnNMRClickList3)
	ON_COMMAND(ID_PM_LTE_CASE_CELL_DEL, &CPM_LTE_Case_Cell::OnPmLteCaseDel)
	ON_COMMAND(ID_PM_LTE_CASE_CELL_M_DEL, &CPM_LTE_Case_Cell::OnPmLteCaseWDel)
	ON_BN_CLICKED(IDC_EXCEL3, &CPM_LTE_Case_Cell::OnBnClickedExcel3)
	ON_NOTIFY(NM_RCLICK, IDC_LIST5, &CPM_LTE_Case_Cell::OnNMRClickList5)
END_MESSAGE_MAP()


// CPM_LTE_Case_Cell 诊断

#ifdef _DEBUG
void CPM_LTE_Case_Cell::AssertValid() const
{
	CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CPM_LTE_Case_Cell::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif
...
// CPM_LTE_Case_Cell 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnInitialUpdate()
{
	CFormView::OnInitialUpdate();

	// TODO: 在此添加专用代码和/或调用基类
	//窗体名称
	v_sFrame_Name="LTE_性能查询模板_小区";								//窗体名称
	...
	m_cList_Case.InsertColumn(3,"小区",LVCFMT_LEFT,200);				//
	m_cList_Case.InsertColumn(4,"创建日期",LVCFMT_CENTER,120);			//
	...
	m_SplitterPane_Main.Create(											//垂直线
		...
		IDC_PM_LTE_Case,		// this ID is used for saving/restoring splitter position and it must be unique !
...
//------------------------------------------------------------------------------------------------------			
BOOL CPM_LTE_Case_Cell::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 在此添加专用代码和/或调用基类
...
//	函数功能：子窗销毁
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnDestroy()
{
	CFormView::OnDestroy();
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：导出
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnBnClickedExcel2()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
...
//	函数功能：列表窗排序
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnLvnColumnclickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：列表点击响应
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnNMClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (pNMListView->iItem>=0)											//判断：条目序号有效？？？
	{
		if (v_pIterator_Frame->second.v_iThread_Exit==0)
		{
			...
			v_sName_Case=m_cList.GetItemText(v_iList_Item,2);			//获取：选择的模板名称
...
//	函数功能：列表右击响应【模板】
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnNMRClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	CMenu	aMenu;														//菜单
	...
	aMenu.LoadMenu(IDR_PM_LTE_CASE_CELL_DEL);								//装载
...
//	函数功能：删除性能查询模板
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnPmLteCaseDel()
{
	// TODO: 在此添加命令处理程序代码
	int				v_iItem,vi;											//临时变量
	...
	if(MessageBox("删除选中的 "+v_sTemp+" 个 性能查询模板_小区， 删除确认 ? ","数据删除确认",MB_OKCANCEL|MB_ICONQUESTION)==IDOK)	//删除确认？？？
	{
		//对所有选中的Item进行操作
		while ((v_iItem = m_cList.GetNextItem(-1, LVNI_SELECTED)) != -1)//条目选中？？？
		{
			...
			//删除：Cell
			v_sSql = "delete from CON_PM_LTE_Case_Cell where zAccount='"
						+v_sAccount+"' and zName='"+v_sName+"'\r\n";	//SQL删除
			v_sSql += "delete from CON_PM_LTE_Case_Cell_Data where zAccount='"
			...
			v_sSql = "Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"
						+v_sCurrentTime+"','性能查询模板_小区_删除','模板名称："
...
//	函数功能：列表右击响应【小区】
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnNMRClickList5(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	CMenu	aMenu;														//菜单
	...
	aMenu.LoadMenu(IDR_PM_LTE_CASE_CELL_M_DEL);							//装载
	CMenu *pPopup =aMenu.GetSubMenu(0);									//生成浮动菜单
	pPopup->TrackPopupMenu(TPM_LEFTALIGN,point.x,point.y,this);			//显示：依据鼠标位置[point]
...
//	函数功能：删除性能查询模板【小区】
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnPmLteCaseWDel()
{
	// TODO: 在此添加命令处理程序代码
	int				v_iItem,vi;											//临时变量
	CString			v_sTemp,v_sSql,v_sName,v_sCell,v_sAccount;			//临时变量
	COleDateTime	v_tTime=COleDateTime::GetCurrentTime();
	CString			v_sCurrentTime=v_tTime.Format("%Y-%m-%d %H:%M:%S");
	...
	if(MessageBox("删除选中的 "+v_sTemp+" 个 性能查询模板_小区， 删除确认 ? ","数据删除确认",MB_OKCANCEL|MB_ICONQUESTION)==IDOK)	//删除确认？？？
	{
		...
		while ((v_iItem = m_cList_Case.GetNextItem(-1, LVNI_SELECTED)) != -1)//条目选中？？？
		{
			...
			v_sCell=m_cList_Case.GetItemText(v_iItem,3);				//获得：小区名称

			//删除：Cell
			v_sSql = "delete from CON_PM_LTE_Case_Cell_Data where zAccount='"+v_sAccount
						+"' and zName='"+v_sName+"' and zCell='"+v_sCell+"'";		//SQL删除
			My_ExecuteSQL(v_sSql);										//写表
			...
			v_sSql = "Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"
				+v_sCurrentTime+"','性能查询模板_小区_删除','模板名称："
				+v_sName+"；小区名称："+v_sCell+" 【操作员："+theApp.a_sUser_Name+"】')";
			My_ExecuteSQL(v_sSql);										//数据库执行
		}
	}
}
...
//	函数功能：查询
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnBnClickedQuary2()
{
	// TODO: 在此添加控件通知处理程序代码
	v_iWorking=1;														//工作任务：查询
...
//	函数功能：导入
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::OnBnClickedExcel3()
{
	// TODO: 在此添加控件通知处理程序代码
	CPM_LTE_Case_Cell_Input	v_cDlg;										//实例化
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CPM_LTE_Case_Cell::My_Thread_Inoput(LPVOID lparam)
{
	CPM_LTE_Case_Cell *  lp_this = NULL ;
	lp_this = (CPM_LTE_Case_Cell *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cell::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 1:
		My_LoadData_Per();												//模板查询
		break;
	case 2:
		My_LoadData_Case();												//小区查询
		break;
	}
}
...
//	函数功能：装载信息【模板】
//------------------------------------------------------------------------------------------------------			
void CPM_LTE_Case_Cell::My_LoadData_Per()
{
	...
	try
	{
		...
		v_sSql += "SELECT * FROM CON_PM_LTE_Case_Cell where zAccount='共享' or zAccount='"
...
//	函数功能：装载信息【小区】
//------------------------------------------------------------------------------------------------------			
void CPM_LTE_Case_Cell::My_LoadData_Case()
{
	int				v_iItem,v_iID=0;									//临时变量
	CString			v_sTemp,v_sSql;										//临时变量
	CString			v_sUser,v_sName,v_sCell,v_sDate;					//临时变量
	COleDateTime	v_dDate;											//临时变量
	...
	try
	{
		...
		v_sSql="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED ";
		v_sSql += "SELECT * FROM CON_PM_LTE_Case_Cell_Data where zAccount='"+v_sName_Account+"' and zName='"+v_sName_Case+"' order by zCell";
		...
		while(!v_pRecordset->adoEOF)
		{
			...
			m_cList_Case.SetItemText(v_iItem,3,v_sCell);				//v_sCell
			m_cList_Case.SetItemText(v_iItem,4,v_sDate);				//v_sDate
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CPM_LTE_Case_Cell::My_ExecuteSQL(CString v_sSql)
{
