...
IMPLEMENT_DYNCREATE(CCON_E_NodeB, CFormView)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CCON_E_NodeB::CCON_E_NodeB()
	: CFormView(CCON_E_NodeB::IDD)
{
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CCON_E_NodeB, CFormView)
	ON_WM_DESTROY()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_QUARY, &CCON_E_NodeB::OnBnClickedQuary)
	ON_BN_CLICKED(IDC_ELEMENT, &CCON_E_NodeB::OnBnClickedElement)
	ON_BN_CLICKED(IDC_EXCEL2, &CCON_E_NodeB::OnBnClickedExcel2)
	ON_WM_CLOSE()
	ON_NOTIFY(NM_CLICK, IDC_LIST3, &CCON_E_NodeB::OnNMClickList3)
	ON_NOTIFY(NM_RCLICK, IDC_LIST3, &CCON_E_NodeB::OnNMRClickList3)
	ON_COMMAND(ID_CON_LTE_NODEB_DEL, &CCON_E_NodeB::OnConLteNodebDel)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST3, &CCON_E_NodeB::OnLvnColumnclickList3)
END_MESSAGE_MAP()
...
#ifdef _DEBUG
void CCON_E_NodeB::AssertValid() const
{
	CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CCON_E_NodeB::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif
...
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::OnInitialUpdate()
{
	CFormView::OnInitialUpdate();

	// TODO: 在此添加专用代码和/或调用基类
	//窗体名称
	v_sFrame_Name="E_NodeB 配置";										//窗体名称
...
//------------------------------------------------------------------------------------------------------			
BOOL CCON_E_NodeB::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 在此添加专用代码和/或调用基类
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：站址导出
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::OnBnClickedExcel2()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
...
//	函数功能：站址导入
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::OnBnClickedElement()
{
	// TODO: 在此添加控件通知处理程序代码
	CCON_E_NodeB_Input	v_cDlg;											//实例化
...
//	函数功能：列表窗排序 [站址]
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::OnLvnColumnclickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：列表点击响应
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::OnNMClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：列表右击响应
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::OnNMRClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	CMenu	aMenu;														//菜单
	...
	aMenu.LoadMenu(IDR_LTE_NODEB_DEL);								//装载：IDR_GSM_BTS菜单
...
//	函数功能：删除基站
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::OnConLteNodebDel()
{
	// TODO: 在此添加命令处理程序代码
	int		v_iItem,vi;													//临时变量
	...
	if(MessageBox("删除选中的 "+v_sTemp+" 个 LTE NodeB， 删除确认 ? ","数据删除确认",MB_OKCANCEL|MB_ICONQUESTION)==IDOK)	//删除确认？？？
	{
		//对所有选中的Item进行操作
		while ((v_iItem = m_cList.GetNextItem(-1, LVNI_SELECTED)) != -1)//条目选中？？？
		{
			...
			CString v_sSql="Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"+v_sCurrentTime+"','删除E_NodeB','E_NodeB名称："
...
//	函数功能：查询
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::OnBnClickedQuary()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CCON_E_NodeB::My_Thread_Inoput(LPVOID lparam)
{
	CCON_E_NodeB *  lp_this = NULL ;
	lp_this = (CCON_E_NodeB *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 1:
...
//	函数功能：查询站址信息
//------------------------------------------------------------------------------------------------------
void CCON_E_NodeB::My_Query_Station()
{
	int				v_iID=0;											//临时变量
	...
	v_sSql += "SELECT LTE_NODEB.* FROM LTE_NODEB where zRNC like '%dalian_%'";
...
//	函数功能：装载站址信息
//------------------------------------------------------------------------------------------------------			
void CCON_E_NodeB::My_LoadData_Station_Data(CString v_sSql)
{
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CCON_E_NodeB::My_ExecuteSQL(CString v_sSql)
{
