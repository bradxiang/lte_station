// PM_LTE_Region.cpp : 实现文件
//

#include "stdafx.h"
#include "Station.h"
#include "PM_LTE_Region.h"
#include "PM_LTE_Region_Input.h"


// CPM_LTE_Region

IMPLEMENT_DYNCREATE(CPM_LTE_Region, CFormView)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CPM_LTE_Region::CPM_LTE_Region()
	: CFormView(CPM_LTE_Region::IDD)
{
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CPM_LTE_Region, CFormView)
	ON_WM_DESTROY()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_QUARY, &CPM_LTE_Region::OnBnClickedQuary)
	ON_BN_CLICKED(IDC_ELEMENT, &CPM_LTE_Region::OnBnClickedElement)
	ON_BN_CLICKED(IDC_EXCEL2, &CPM_LTE_Region::OnBnClickedExcel2)
	ON_WM_CLOSE()
	ON_COMMAND(ID_PM_LTE_REGION_DEL, &CPM_LTE_Region::OnConLteCellDel)
	ON_NOTIFY(NM_RCLICK, IDC_LIST3, &CPM_LTE_Region::OnNMRClickList3)
	ON_NOTIFY(NM_CLICK, IDC_LIST3, &CPM_LTE_Region::OnNMClickList3)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST3, &CPM_LTE_Region::OnLvnColumnclickList3)
END_MESSAGE_MAP()
...
#ifdef _DEBUG
void CPM_LTE_Region::AssertValid() const
{
	CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CPM_LTE_Region::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif
...
// CPM_LTE_Region 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::OnInitialUpdate()
{
	CFormView::OnInitialUpdate();

	// TODO: 在此添加专用代码和/或调用基类
	//窗体名称
	v_sFrame_Name="LTE_性能管理_区域";									//窗体名称
...
//------------------------------------------------------------------------------------------------------			
BOOL CPM_LTE_Region::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 在此添加专用代码和/或调用基类
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：站址导出
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::OnBnClickedExcel2()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
...
//	函数功能：站址导入
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::OnBnClickedElement()
{
	// TODO: 在此添加控件通知处理程序代码
	CPM_LTE_Region_Input	v_cDlg;											//实例化
...
//	函数功能：列表点击响应
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::OnNMClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：列表右击响应
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::OnNMRClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	CMenu	aMenu;														//菜单
	...
	aMenu.LoadMenu(IDR_PM_LTE_REGION_DEL);							//装载：右键菜单
...
//	函数功能：删除性能
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::OnConLteCellDel()
{
	// TODO: 在此添加命令处理程序代码
	int		v_iItem,vi;													//临时变量
	...
	if(MessageBox("删除选中的 "+v_sTemp+" 个 性能， 删除确认 ? ","数据删除确认",MB_OKCANCEL|MB_ICONQUESTION)==IDOK)	//删除确认？？？
	{
		...
		while ((v_iItem = m_cList_Station.GetNextItem(-1, LVNI_SELECTED)) != -1)//条目选中？？？
		{
			...
			//删除：Cell
			v_sSql = "delete from CON_PM_LTE_Region where zPm='"+v_sCell+"'";	//SQL删除
			...
			CString v_sSql="Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"+v_sCurrentTime+"','删除性能(区域)','性能名称："
...
//	函数功能：列表窗排序 [性能]
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::OnLvnColumnclickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：查询
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::OnBnClickedQuary()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CPM_LTE_Region::My_Thread_Inoput(LPVOID lparam)
{
	CPM_LTE_Region *  lp_this = NULL ;
	lp_this = (CPM_LTE_Region *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 1:
...
//	函数功能：查询站址信息
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Region::My_Query_Station()
{
	int				v_iID=0;											//临时变量
	...
	v_sSql += "SELECT * FROM CON_PM_LTE_Region"; 
...
//	函数功能：装载站址信息
//------------------------------------------------------------------------------------------------------			
void CPM_LTE_Region::My_LoadData_Station_Data(CString v_sSql)
{
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CPM_LTE_Region::My_ExecuteSQL(CString v_sSql)
{
