...
IMPLEMENT_DYNCREATE(CPm_GSM_Cell_HO, CFormView)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CPm_GSM_Cell_HO::CPm_GSM_Cell_HO()
	: CFormView(CPm_GSM_Cell_HO::IDD)
{
}
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CPm_GSM_Cell_HO, CFormView)
	ON_WM_DESTROY()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_QUARY, &CPm_GSM_Cell_HO::OnBnClickedQuary)
	ON_BN_CLICKED(IDC_EXCEL2, &CPm_GSM_Cell_HO::OnBnClickedExcel2)
	ON_WM_CLOSE()
	ON_NOTIFY(NM_CLICK, IDC_LIST3, &CPm_GSM_Cell_HO::OnNMClickList3)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST3, &CPm_GSM_Cell_HO::OnLvnColumnclickList3)
	ON_CBN_CLOSEUP(IDC_COMBO4, &CPm_GSM_Cell_HO::OnCbnCloseupCombo4)
	ON_BN_CLICKED(IDC_EXCEL3, &CPm_GSM_Cell_HO::OnBnClickedExcel3)
	ON_BN_CLICKED(IDC_EXCEL4, &CPm_GSM_Cell_HO::OnBnClickedExcel4)
	ON_BN_CLICKED(IDC_EXCEL5, &CPm_GSM_Cell_HO::OnBnClickedExcel5)
END_MESSAGE_MAP()
...
#ifdef _DEBUG
void CPm_GSM_Cell_HO::AssertValid() const
{
	CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CPm_GSM_Cell_HO::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif
...
// CPm_GSM_Cell_HO 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnInitialUpdate()
{
	CFormView::OnInitialUpdate();

	// TODO: 在此添加专用代码和/或调用基类
	//窗体名称
	v_sFrame_Name="GSM_性能计算_CELL_HO";								//窗体名称
...
//------------------------------------------------------------------------------------------------------			
BOOL CPm_GSM_Cell_HO::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 在此添加专用代码和/或调用基类
...
//	函数功能：子窗销毁
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnDestroy()
{
	CFormView::OnDestroy();
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：导出
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnBnClickedExcel2()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
...
//	函数功能：类型选择
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnCbnCloseupCombo4()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：列表点击响应
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnNMClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：列表窗排序
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnLvnColumnclickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：增加
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnBnClickedExcel3()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：修改
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnBnClickedExcel4()
{
	// TODO: 在此添加控件通知处理程序代码
	if (v_iList_Item==-1)
	{
...
//	函数功能：删除
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnBnClickedExcel5()
{
	// TODO: 在此添加控件通知处理程序代码
	CString v_sSql;														//临时变量
...
//	函数功能：查询
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::OnBnClickedQuary()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CPm_GSM_Cell_HO::My_Thread_Inoput(LPVOID lparam)
{
	CPm_GSM_Cell_HO *  lp_this = NULL ;
	lp_this = (CPm_GSM_Cell_HO *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 1:
...
//	函数功能：查询站址信息
//------------------------------------------------------------------------------------------------------
void CPm_GSM_Cell_HO::My_Query_Station()
{
	int				v_iID=0;											//临时变量
	...
	if (m_cType.GetCurSel()==0)											//性能组
	{
		v_sSql += "SELECT * FROM A_CAPGROUP_Info where ztypename not like '%BSC%'"; 
	...
	else
	{
		v_sSql += "SELECT znum,PM_G.zgroupname,PM_G.ztypename,zcap,zformula,zobjtypes,zunit,zmark FROM A_CAP_Info \
				  left join A_CAPGROUP_Info as PM_G on PM_G.zgroupnum=A_CAP_Info.zgroupnum where ztypename not like '%BSC%'"; 
...
//	函数功能：装载信息
//------------------------------------------------------------------------------------------------------			
void CPm_GSM_Cell_HO::My_LoadData_Station_Data(CString v_sSql)
{
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CPm_GSM_Cell_HO::My_ExecuteSQL(CString v_sSql)
{
