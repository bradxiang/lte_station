...
// CPM_LTE_Case_PM_Input 对话框

IMPLEMENT_DYNAMIC(CPM_LTE_Case_PM_Input, CDialog)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CPM_LTE_Case_PM_Input::CPM_LTE_Case_PM_Input(CWnd* pParent /*=NULL*/)
	: CDialog(CPM_LTE_Case_PM_Input::IDD, pParent)
{
...
//	函数功能：析构函数
//------------------------------------------------------------------------------------------------------
CPM_LTE_Case_PM_Input::~CPM_LTE_Case_PM_Input()
{
}
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_PM_Input::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CPM_LTE_Case_PM_Input, CDialog)
	ON_WM_SIZE()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BUTTON4, &CPM_LTE_Case_PM_Input::OnBnClickedButton4)
	ON_BN_CLICKED(IDC_BUTTON1, &CPM_LTE_Case_PM_Input::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON6, &CPM_LTE_Case_PM_Input::OnBnClickedButton6)
END_MESSAGE_MAP()


// CPM_LTE_Case_PM_Input 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
BOOL CPM_LTE_Case_PM_Input::OnInitDialog()
{
	...
	m_cList.InsertColumn(3,"类型",LVCFMT_LEFT,100);						//
	m_cList.InsertColumn(4,"性能",LVCFMT_LEFT,280);						//
	...
	m_cList.InsertColumn(8,"创建日期",LVCFMT_CENTER,120);				//
	...
	m_SplitterPane_Main.Create(											//垂直线
		...
		IDC_PM_LTE_Case_PM_Input,	// this ID is used for saving/restoring splitter position and it must be unique !
...
// 消息预处理：把回车键变换为TAB
//------------------------------------------------------------------------------------------------------			
BOOL CPM_LTE_Case_PM_Input::PreTranslateMessage(MSG* pMsg)
{
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_PM_Input::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：关闭窗体
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_PM_Input::OnClose()
{
...
//	函数功能：异常导出
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_PM_Input::OnBnClickedButton6()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
	MyExcel.Show_Name(&m_cList_Bug,"导入异常_性能查询模板_性能",m_cFile_Open.GetCheck());		//导出Excel
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：获取资管数据CSV文件信息
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_PM_Input::OnBnClickedButton4()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：把列表窗中的数据保存到数据表
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_PM_Input::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CPM_LTE_Case_PM_Input::My_Thread_Inoput(LPVOID lparam)
{
	CPM_LTE_Case_PM_Input *  lp_this = NULL ;
	lp_this = (CPM_LTE_Case_PM_Input *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_PM_Input::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 0:
...
//	函数功能：获取CSV文件信息
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_PM_Input::My_File()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	if(filedialog->DoModal()==IDOK)										//打开选择文件对话窗
	{
		...
		v_mMap.insert(pair<CString,int>("用户账号",1));					//1：用户账号
		v_mMap.insert(pair<CString,int>("模板名称",2));					//2：模板名称
		v_mMap.insert(pair<CString,int>("类型",3));						//3：类型
		v_mMap.insert(pair<CString,int>("性能",4));						//4：性能
		...
		v_mMap.insert(pair<CString,int>("创建日期",8));					//8：创建日期

		if (!Get_CSV_Cell(v_sFilename,&v_mMap,true))					//获取：CSV文件信息
			MessageBox("无有效的性能查询模板_性能文件，请重新选择 ！！！");		//窗体提示	
	}
	delete(filedialog);
	v_mMap.clear();														//清除容器
}
...
//----------------------------------------------------------------------------------------------------
bool CPM_LTE_Case_PM_Input::Get_CSV_Cell(CString v_sFile,mString_Int *v_mHead,bool v_bHead)
{
	CStdioFile	v_fRead;												//实例化	
	CString		v_sStr,v_sTemp;											//临时变量
	bool		v_Flag_Head=true;										//临时变量
	int			v_iLen,v_iNum,v_iItem,v_iSum;							//临时变量
	CString		v_sAccount,v_sName,v_sType,v_sPM_Name;					//临时变量
	...
	while(v_fRead.ReadString(v_sStr))									//数据读取：一行
	{
		...
		while (true)
		{
			...
			if (v_pCol != v_mMap.end())									//找到？？？
			{
				...
				if (v_pCol->second==1)									//
					v_sAccount=v_sTemp;									//用户账号-
				else if (v_pCol->second==2)
					v_sName=v_sTemp;									//模板
				else if (v_pCol->second==3)
					v_sType=v_sTemp;									//类型
				else if (v_pCol->second==4)
					v_sPM_Name=v_sTemp;									//性能
			}
			v_iNum++;													//列号：++
		}
		if (!v_sAccount.IsEmpty() && !v_sName.IsEmpty() && (v_sAccount==theApp.a_sUser_Account || v_sAccount=="共享"))	//条件：数据有效？？？
		{
			v_pRepeat=v_mRepeat.find(v_sAccount+v_sName+v_sType+v_sPM_Name);		//存在？？？
			if (v_pRepeat == v_mRepeat.end())							//不存在
			{
				v_mRepeat.insert(pair<CString,int>(v_sAccount+v_sName+v_sType+v_sPM_Name,0));	//容器：增加
			...
			{
				My_ShowList_Bug("查询模板导入异常","异常：性能重复 【模板名称："+v_sName+"；性能："+v_sPM_Name+"】");		//显示：异常信息
			}
		}
		v_mData.clear();												//容器：清除
	}
...
//	函数功能：性能数据导入
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_PM_Input::My_Input_Cell()
{
	int		v_iLen,vi;													//临时变量
	CString	v_sSql,v_sTemp,v_sNew;										//临时变量
	CString	v_sName,v_sType,v_sPM_Name,v_sAccount,v_sCurTime;			//临时变量
	...
	if(MessageBox("模板数据 导入确认 ? ","数据导入确认",MB_OKCANCEL|MB_ICONQUESTION)!=IDOK)	//导入确认？？？
		return;															//返回：退出
	...
	if (v_iLen<=0)
	{
		MessageBox("没有可以导入的模板数据 ！！！");					//返回：无数据
		return;
	}

	//【2】获取相关数据 [模板]
	My_Load_PM(&v_mPM,&v_mCase,&v_mPM_Type);							//获取：信息 [放入：v_mPM]

	//【3】数据表保存 [存在：更新；不存在：增加]
	...
	for (vi=0;vi<v_iLen;vi++)
	{
		...
		//获取：数据
		v_sAccount=m_cList.GetItemText(vi,1);							//帐号
		v_sName=m_cList.GetItemText(vi,2);								//模板
		v_sType=m_cList.GetItemText(vi,3);								//类型
		v_sPM_Name=m_cList.GetItemText(vi,4);							//性能
		...
		{																//异常性能类型及名称
			...
			My_ShowList_Bug("模板导入异常","异常：性能类型或名称 【类型："+v_sType+"；性能名称："+v_sPM_Name+"】");		//显示：异常信息
			continue;
		}

		v_iSum++;														//总数：小区
		v_pIter=v_mPM.find(v_sAccount+v_sName+v_sType+v_sPM_Name);		//查找：模板
		if (v_pIter == v_mPM.end())										//没找到？？？
		{																//新增
			v_sNew="Insert into CON_Pm_LTE_Case_PM_Data(zAccount,zName,zType,zPM_Name,zAlarm_Flag,zAlarm_Value,zAlarm_Type,zDate) values ('"
					+v_sAccount+"','"+v_sName+"','"+v_sType+"','"+v_sPM_Name+"','"+v_sAlarm_Flag
					+"','"+v_sAlarm_Value+"','"+v_sAlarm_Type+"','"+v_sCurTime+"')";	//SQL
			v_sSql+=v_sNew+"\r\n";										//增加一条SQL
			v_iSum_Sql++;												//SQL数量++
		...
		{
			v_sNew="Update CON_Pm_LTE_Case_PM_Data set zAlarm_Flag='"+v_sAlarm_Flag+"',zAlarm_Value='"
				+v_sAlarm_Value+"',zAlarm_Type='"+v_sAlarm_Type+"' where zAccount='"+v_sAccount
				+"' and zName='"+v_sName+"' and zType='"+v_sType+"' and zPM_Name='"+v_sPM_Name+"'";		//SQL
			v_sSql+=v_sNew+"\r\n";										//增加一条SQL
			v_iSum_Sql++;												//SQL数量++
			m_cList.SetItemBkColor(vi,RGB(67,240,250));					//设置：背景色 [亮绿色]			
		}
		//增加模板
		v_pIter=v_mCase.find(v_sAccount+v_sName);						//查找：模板
		if (v_pIter == v_mCase.end())									//没找到？？？
		{
	...
	//【5】日志
	v_sSql="Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"+v_sCurrentTime+"','查询模板 数据导入','[用户："+theApp.a_sUser_Name+"]')";//SQL
	...
	MessageBox("性能模板数据导入完成："+v_sTemp);						//返回：无数据
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：装载信息【性能模板】
//------------------------------------------------------------------------------------------------------			
void CPM_LTE_Case_PM_Input::My_Load_PM(mString_Int *v_mPM,mString_Int *v_mCase,mString_Int *v_mPM_Type)
{
	CString			v_sSql;												//临时变量
	CString			v_sTemp,v_sName,v_sType,v_sPM_Name,v_sAccount;		//临时变量
	...
	try
	{
		...
		v_sSql += "SELECT * FROM CON_Pm_LTE_Case_PM_Data where zAccount='共享' or zAccount='"+theApp.a_sUser_Account+"'";
		v_pRecordset = v_cAdoConn.GetRecordSet((_bstr_t)v_sSql);		//执行SELETE语句										
		while(!v_pRecordset->adoEOF)
		{
			v_sAccount = v_cAdoConn.GetField_String("zAccount");		//帐号
			v_sName = v_cAdoConn.GetField_String("zName");				//模板
			v_sType = v_cAdoConn.GetField_String("zType");				//类型
			v_sPM_Name = v_cAdoConn.GetField_String("zPM_Name");		//性能

			v_mPM->insert(pair<CString,int>(v_sAccount+v_sName+v_sType+v_sPM_Name,0));		//容器：增加
...
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_PM_Input::My_ShowList_Bug(CString v_sBug_Type,CString v_sBug)
{
	int		v_iIndex;													//临时变量
	CString v_sTemp;													//临时变量
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CPM_LTE_Case_PM_Input::My_ExecuteSQL(CString v_sSql)
{
