// PM_LTE_Case_Cal.cpp : 实现文件
//

#include "stdafx.h"
#include "Station.h"
#include "PM_LTE_Case_Cal.h"
#include "PM_LTE_Case_Cal_Input.h"


// CPM_LTE_Case_Cal

IMPLEMENT_DYNCREATE(CPM_LTE_Case_Cal, CFormView)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CPM_LTE_Case_Cal::CPM_LTE_Case_Cal()
	: CFormView(CPM_LTE_Case_Cal::IDD)
{
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CPM_LTE_Case_Cal, CFormView)
	ON_WM_DESTROY()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_QUARY, &CPM_LTE_Case_Cal::OnBnClickedQuary)
	ON_BN_CLICKED(IDC_ELEMENT, &CPM_LTE_Case_Cal::OnBnClickedElement)
	ON_BN_CLICKED(IDC_EXCEL2, &CPM_LTE_Case_Cal::OnBnClickedExcel2)
	ON_WM_CLOSE()
	ON_COMMAND(ID_PM_LTE_CASE_CAL_DEL, &CPM_LTE_Case_Cal::OnConLteCellDel)
	ON_NOTIFY(NM_RCLICK, IDC_LIST3, &CPM_LTE_Case_Cal::OnNMRClickList3)
	ON_NOTIFY(NM_CLICK, IDC_LIST3, &CPM_LTE_Case_Cal::OnNMClickList3)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST3, &CPM_LTE_Case_Cal::OnLvnColumnclickList3)
END_MESSAGE_MAP()
...
#ifdef _DEBUG
void CPM_LTE_Case_Cal::AssertValid() const
{
	CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CPM_LTE_Case_Cal::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif
...
// CPM_LTE_Case_Cal 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::OnInitialUpdate()
{
	CFormView::OnInitialUpdate();

	// TODO: 在此添加专用代码和/或调用基类
	//窗体名称
	v_sFrame_Name="LTE_性能查询模板_计算";								//窗体名称
	...
	m_cList_Station.InsertColumn(1,"用户账号",LVCFMT_LEFT,100);			//
	m_cList_Station.InsertColumn(2,"模板名称",LVCFMT_LEFT,120);			//
	m_cList_Station.InsertColumn(3,"类型",LVCFMT_LEFT,100);				//
	m_cList_Station.InsertColumn(4,"计算公式",LVCFMT_LEFT,400);			//
	m_cList_Station.InsertColumn(5,"创建日期",LVCFMT_CENTER,120);		//
...
//------------------------------------------------------------------------------------------------------			
BOOL CPM_LTE_Case_Cal::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 在此添加专用代码和/或调用基类
...
//	函数功能：子窗销毁
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::OnDestroy()
{
	CFormView::OnDestroy();
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：站址导出
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::OnBnClickedExcel2()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
...
//	函数功能：站址导入
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::OnBnClickedElement()
{
	// TODO: 在此添加控件通知处理程序代码
	CPM_LTE_Case_Cal_Input	v_cDlg;											//实例化
...
//	函数功能：列表点击响应
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::OnNMClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：列表右击响应
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::OnNMRClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	CMenu	aMenu;														//菜单
	...
	aMenu.LoadMenu(IDR_PM_LTE_CASE_CAL_DEL);							//装载：右键菜单
...
//	函数功能：删除性能
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::OnConLteCellDel()
{
	// TODO: 在此添加命令处理程序代码
	int		v_iItem,vi;													//临时变量
	CString	v_sAccount,v_sTemp,v_sSql,v_sName;							//临时变量

	vi = m_cList_Station.GetSelectedCount();							//选中条目数量
	v_sTemp.Format("%d",vi);											//字符串
	if(MessageBox("删除选中的 "+v_sTemp+" 个 模板， 删除确认 ? ","数据删除确认",MB_OKCANCEL|MB_ICONQUESTION)==IDOK)	//删除确认？？？
	{
		//对所有选中的Item进行操作
		while ((v_iItem = m_cList_Station.GetNextItem(-1, LVNI_SELECTED)) != -1)//条目选中？？？
		{
			v_sAccount=m_cList_Station.GetItemText(v_iItem,1);			//获得：帐号名称
			v_sName=m_cList_Station.GetItemText(v_iItem,2);				//获得：模板名称

			//删除：Cell
			v_sSql = "delete from CON_PM_LTE_Case_Cal where zAccount='"+v_sAccount+"' and zName='"+v_sName+"'";		//SQL删除
			...
			CString v_sSql="Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"+v_sCurrentTime+"','删除自定义计算性能','性能名称："
...
//	函数功能：列表窗排序 [站址]
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::OnLvnColumnclickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：查询
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::OnBnClickedQuary()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CPM_LTE_Case_Cal::My_Thread_Inoput(LPVOID lparam)
{
	CPM_LTE_Case_Cal *  lp_this = NULL ;
	lp_this = (CPM_LTE_Case_Cal *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 1:
		My_Query_Station();												//自定义计算信息
		break;
	}
}
...
//	函数功能：查询自定义计算信息
//------------------------------------------------------------------------------------------------------
void CPM_LTE_Case_Cal::My_Query_Station()
{
	int				v_iID=0;											//临时变量
	...
	v_sSql += "SELECT * FROM CON_PM_LTE_Case_Cal"; 
	...
	if (!v_sLog_Key.IsEmpty())											//查询KEY处理
	{
		...
		{
		case 0:															//模板账号
			v_sSql+=" where zName like '%"+v_sLog_Key+"%'";
			break;
		case 1:															//计算公式
		...
		v_sSql+=" and zAccount='"+theApp.a_sUser_Account+"'";			//帐号
	}
	else
		v_sSql+=" where zAccount='"+theApp.a_sUser_Account+"'";			//帐号

	v_sSql+=" order by zName";
...
//	函数功能：装载站址信息
//------------------------------------------------------------------------------------------------------			
void CPM_LTE_Case_Cal::My_LoadData_Station_Data(CString v_sSql)
{
	int				v_iItem,v_iID=0;									//临时变量
	CString			v_sTemp,v_sDate;									//临时变量
	CString			v_sAccount,v_sName,v_sFormula,v_sType,v_sAlarm_Flag;//临时变量
	...
	try
	{
		...
		while(!v_pRecordset->adoEOF)
		{
			...
			m_cList_Station.SetItemText(v_iItem,1,v_sAccount);			//帐号
			m_cList_Station.SetItemText(v_iItem,2,v_sName);				//模板名称
			m_cList_Station.SetItemText(v_iItem,3,v_sType);				//类型
			m_cList_Station.SetItemText(v_iItem,4,v_sFormula);			//计算公式
			m_cList_Station.SetItemText(v_iItem,5,v_sDate);				//日期
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CPM_LTE_Case_Cal::My_ExecuteSQL(CString v_sSql)
{
