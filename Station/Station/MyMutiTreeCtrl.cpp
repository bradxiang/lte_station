...
//------------------------------------------------------------------------------------------------------
//	函数功能：点击鼠标左键，获得子节点 【可以获得状态选择的消息】
//	工	  作：有效节点，发送消息；(this->GetParent())->PostMessage(WM_USER + 12,0,(LPARAM)hItem);
//------------------------------------------------------------------------------------------------------
...
//	函数功能：树展开n级
//------------------------------------------------------------------------------------------------------
void CMyMutiTreeCtrl::ExpandTree_Level(HTREEITEM v_hNode,int v_iNum)
{
	if (v_iNum==0)
		return;
	...
	{
		ExpandTree_Level(hNextItem,v_iNum);
...
//	函数功能：树收缩所有节点
//------------------------------------------------------------------------------------------------------
void CMyMutiTreeCtrl::CloseTree(HTREEITEM v_hNode)
{
	if(!ItemHasChildren(v_hNode))
		return;
	HTREEITEM hNextItem = GetChildItem(v_hNode);
