// CON_LTE_Cell_Input.cpp : 实现文件
//

#include "stdafx.h"
#include "Station.h"
#include "CON_LTE_Cell_Input.h"


// CCON_LTE_Cell_Input 对话框

IMPLEMENT_DYNAMIC(CCON_LTE_Cell_Input, CDialog)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CCON_LTE_Cell_Input::CCON_LTE_Cell_Input(CWnd* pParent /*=NULL*/)
	: CDialog(CCON_LTE_Cell_Input::IDD, pParent)
{
...
//	函数功能：析构函数
//------------------------------------------------------------------------------------------------------
CCON_LTE_Cell_Input::~CCON_LTE_Cell_Input()
{
}
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell_Input::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CCON_LTE_Cell_Input, CDialog)
	ON_WM_SIZE()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BUTTON4, &CCON_LTE_Cell_Input::OnBnClickedButton4)
	ON_BN_CLICKED(IDC_BUTTON1, &CCON_LTE_Cell_Input::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON6, &CCON_LTE_Cell_Input::OnBnClickedButton6)
END_MESSAGE_MAP()


// CCON_LTE_Cell_Input 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
BOOL CCON_LTE_Cell_Input::OnInitDialog()
{
	...
	m_cList.InsertColumn(3,"E_NodeB",LVCFMT_LEFT,200);					//
	m_cList.InsertColumn(4,"LAC",LVCFMT_LEFT,80);						//
	...
	m_SplitterPane_Main.Create(											//垂直线
		...
		IDC_CON_LTE_Cell_Input,	// this ID is used for saving/restoring splitter position and it must be unique !
...
// 消息预处理：把回车键变换为TAB
//------------------------------------------------------------------------------------------------------			
BOOL CCON_LTE_Cell_Input::PreTranslateMessage(MSG* pMsg)
{
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell_Input::OnSize(UINT nType, int cx, int cy)
{
...
//	函数功能：关闭窗体
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell_Input::OnClose()
{
...
//	函数功能：异常导出
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell_Input::OnBnClickedButton6()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
	MyExcel.Show_Name(&m_cList_Bug,"导入异常_LTE_Cell",m_cFile_Open.GetCheck());		//导出Excel
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：获取资管数据CSV文件信息
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell_Input::OnBnClickedButton4()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：把列表窗中的数据保存到数据表
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell_Input::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动
//------------------------------------------------------------------------------------------------------
UINT CCON_LTE_Cell_Input::My_Thread_Inoput(LPVOID lparam)
{
	CCON_LTE_Cell_Input *  lp_this = NULL ;
	lp_this = (CCON_LTE_Cell_Input *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell_Input::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 0:
...
//	函数功能：获取CSV文件信息
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell_Input::My_File()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	if(filedialog->DoModal()==IDOK)										//打开选择文件对话窗
	{
		...
		v_mMap.insert(pair<CString,int>("E-UTRANCELL名称",2));			//2：中文名
		v_mMap.insert(pair<CString,int>("所属E-NODEB",3));				//3：E-NODEB
		v_mMap.insert(pair<CString,int>("跟踪区码TAC",4));				//4：LAC
...
//----------------------------------------------------------------------------------------------------
bool CCON_LTE_Cell_Input::Get_CSV_Cell(CString v_sFile,mString_Int *v_mHead,bool v_bHead)
{
...
//	函数功能：Cell数据导入
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell_Input::My_Input_Cell()
{
	int		v_iLen,vi;													//临时变量
	CString	v_sSql,v_sTemp,v_sRegion,v_sCell_Name;						//临时变量
	...
	for (vi=0;vi<v_iLen;vi++)
	{
		...
		if (!v_sBTS.IsEmpty())											//有效？？？
		{
			...
			if (v_pIter_BTS != v_mBTS.end())							//找到？？？
			{
				...
				{														//新增
					v_sNew="Insert into LTE_Cell(zCell,zCell_Name,zNODEB,zStation,zLAC,zType,zRegion,zOnline,zDate) values ('"
							+v_sCell+"','"+v_sCell_Name+"','"+v_sBTS+"','"+v_sStation+"','"+v_sLAC+"','"+v_sType+"','"
							+v_sRegion+"','"+v_sOnline+"','"+v_sCurrentTime+"')";		//SQL
					m_cList.SetItemBkColor(vi,RGB(0,255,0));			//设置：背景色 [绿色]
				}
				else
				{
					v_sNew="Update LTE_Cell set zNODEB='"+v_sBTS+"',zCell_Name='"+v_sCell_Name+"',zStation='"+v_sStation
						+"',zLAC='"+v_sLAC+"',zType='"+v_sType+"',zRegion='"+v_sRegion+"',zOnline='"+v_sOnline+"',zDate='"
						+v_sCurrentTime+"' where zCell='"+v_sCell+"'";					//SQL
			...
			{
				My_ShowList_Bug("Cell异常","异常：E_NodeB不存在；【Cell名称："+v_sCell+"； 所属E_NodeB："+v_sBTS+"  不存在】");	//显示：异常信息
				m_cList.SetItemBkColor(vi,RGB(255,0,0));				//设置：背景色 [红色]
			}
		}
		else
		{
			My_ShowList_Bug("Cell异常","异常：E_NodeB名称异常；【Cell名称："+v_sCell+"； 所属E_NodeB："+v_sBTS+"】");			//显示：异常信息
			m_cList.SetItemBkColor(vi,RGB(255,0,0));					//设置：背景色 [红色]
		}
	...
	//日志
	v_sSql="Insert into BAS_Log(zTime,zLog_Type,zLog_Content) values('"+v_sCurrentTime+"','LTE Cell 数据导入','[用户："+theApp.a_sUser_Name+"]')";//SQL
	My_ExecuteSQL(v_sSql);												//数据库执行

	v_sTemp.Format("%d",v_iSum);										//总数：行
	m_cSum.SetWindowText(v_sTemp);										//总数：行
	v_sTemp.Format("LTE Cell总数：%d",v_iSum);
	MessageBox("LTE Cell数据导入完成："+v_sTemp);						//返回：无数据
}

//------------------------------------------------------------------------------------------------------			
//	函数功能：装载信息【Cell】
//------------------------------------------------------------------------------------------------------			
void CCON_LTE_Cell_Input::My_Load_Cell(mString_Int *v_mCell)
{
	CString			v_sSql;												//临时变量
...
//	函数功能：装载信息【BTS】
//------------------------------------------------------------------------------------------------------			
void CCON_LTE_Cell_Input::My_Load_BTS(mString_Int *v_mBTS)
{
	CString			v_sSql;												//临时变量
	CString			v_sTemp,v_sName;									//临时变量
...
//------------------------------------------------------------------------------------------------------
void CCON_LTE_Cell_Input::My_ShowList_Bug(CString v_sBug_Type,CString v_sBug)
{
	int		v_iIndex;													//临时变量
	CString v_sTemp;													//临时变量
...
//	参数：v_sSql：SQL命令；
//------------------------------------------------------------------------------------------------------
bool CCON_LTE_Cell_Input::My_ExecuteSQL(CString v_sSql)
{
