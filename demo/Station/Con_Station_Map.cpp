// Con_Station_Map.cpp : 实现文件
//

#include "stdafx.h"
#include "Station.h"
#include "Con_Station_Map.h"
#include "WebPage.h"													//封装了类CWebPage，用来和HTML，JSP交互
#include "comdef.h" 
#include <atlbase.h>

IMPLEMENT_DYNCREATE(CCon_Station_Map, CFormView)

//------------------------------------------------------------------------------------------------------			
//	函数功能：构造函数
//------------------------------------------------------------------------------------------------------
CCon_Station_Map::CCon_Station_Map()
	: CFormView(CCon_Station_Map::IDD)
{
	v_bShow=false;
...
//	函数功能：屏幕数据映射
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::DoDataExchange(CDataExchange* pDX)
{
...
//	函数功能：屏幕消息映射
//------------------------------------------------------------------------------------------------------
BEGIN_MESSAGE_MAP(CCon_Station_Map, CFormView)
	ON_WM_DESTROY()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_QUARY, &CCon_Station_Map::OnBnClickedQuary)
	ON_WM_CLOSE()
	ON_NOTIFY(NM_CLICK, IDC_LIST3, &CCon_Station_Map::OnNMClickList3)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST3, &CCon_Station_Map::OnLvnColumnclickList3)
	ON_BN_CLICKED(IDC_EXCEL, &CCon_Station_Map::OnBnClickedExcel)
	ON_BN_CLICKED(IDC_BUTTON2, &CCon_Station_Map::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_QUARY3, &CCon_Station_Map::OnBnClickedQuary3)
	ON_BN_CLICKED(IDC_EXCEL2, &CCon_Station_Map::OnBnClickedExcel2)
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
#ifdef _DEBUG
void CCon_Station_Map::AssertValid() const
{
	CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CCon_Station_Map::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif
...
// CCon_Station_Map 消息处理程序

//------------------------------------------------------------------------------------------------------			
//	函数功能：对话窗初始化
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnInitialUpdate()
{
	CFormView::OnInitialUpdate();

	// TODO: 在此添加专用代码和/或调用基类
	//窗体名称
	v_sFrame_Name="离线地图";											//窗体名称
	...
	m_SplitterPane_Main.Create(											//垂直线
		...
		&m_webBrowser,			// Top pane
		&m_cList_Station,			// Bottom pane
	...
	//MAP初始化
	//CHAR v_sPath[255];													//路径
	//memset(v_sPath, 0, sizeof(CHAR) * 255);								//清空
	//GetCurrentDirectory(255, v_sPath);									//获得当前目录
	//StrCat(v_sPath, _T("\\dalian0.html"));								//字符串串接，得到完整目录
	//m_webBrowser.Navigate(v_sPath,NULL,NULL,NULL,NULL);					//加载html页面
    m_webBrowser.Navigate("http://192.168.0.39:9003/index.html", NULL, NULL, NULL, NULL);	//加载html页面
	//显示标志
...
//------------------------------------------------------------------------------------------------------			
BOOL CCon_Station_Map::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 在此添加专用代码和/或调用基类
...
//	函数功能：定时器
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnTimer(UINT_PTR nIDEvent)
{
...
//	函数功能：子窗销毁
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnDestroy()
{
	CFormView::OnDestroy();
...
//	函数功能：调整窗体大小
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnSize(UINT nType, int cx, int cy)
{
	...
	{
		CRect lpRec;													//浏览器尺寸调整
		m_webBrowser.GetWindowRect(&lpRec); 
		ScreenToClient(&lpRec); 

		//调整地图网页尺寸
		CString		v_sWidth,v_sHeight;									//临时变量
		v_sWidth.Format("%d",lpRec.right-lpRec.left-24);				//地图：宽度
		v_sHeight.Format("%d",lpRec.bottom-lpRec.top-6);				//地图：高度
		CWebPage	web;												//生成一个页面
		web.SetDocument(m_webBrowser.GetDocument());					//赋值：网页指针
		web.CallJScript("mySet_Size",v_sWidth,v_sHeight);				//调用JS：调整地图网页尺寸
		m_webBrowser.UpdateData();										//网页：更新
	}
	*/
}
...
//	函数功能：列表窗排序 [站址]
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnLvnColumnclickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：绘制Marker
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
	My_Marker();
}
...
//	函数功能：清除Marker
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnBnClickedExcel()
{
	// TODO: 在此添加控件通知处理程序代码
	CWebPage	web;													//生成一个页面
	web.SetDocument(m_webBrowser.GetDocument());						//赋值：网页指针
...
//	函数功能：站址导出
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnBnClickedExcel2()
{
	// TODO: 在此添加控件通知处理程序代码
	CMyExcel MyExcel;
...
//	函数功能：点击列表窗 [站址]
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnNMClickList3(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//	函数功能：地图获取【站址】
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnBnClickedQuary3()
{
	My_Map_Station();
}
...
//	函数功能：查询【站址】
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnBnClickedQuary()
{
	// TODO: 在此添加控件通知处理程序代码
...
//	函数功能：线程启动	[静态门限计算]
//------------------------------------------------------------------------------------------------------
UINT CCon_Station_Map::My_Thread_Inoput(LPVOID lparam)
{
	CCon_Station_Map *  lp_this = NULL ;
	lp_this = (CCon_Station_Map *)lparam;
...
//	函数功能：导入_主程序
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::My_Input_Main()
{
	switch(v_iWorking)
	{
	case 1:
...
//	函数功能：绘制Marker
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::My_Marker()
{
	// TODO: 在此添加控件通知处理程序代码
	CWebPage	web;													//生成一个页面
	web.SetDocument(m_webBrowser.GetDocument());						//赋值：网页指针
...
//	函数功能：查询机房信息
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::My_Map_Station()
{
	int				v_iID = 0;												//KPI数量
...
//	函数功能：查询机房信息
//------------------------------------------------------------------------------------------------------
void CCon_Station_Map::My_Query_Station()
{
	int				v_iID=0;											//KPI数量
	CString			v_sSql,v_sTemp;										//临时变量
	CString			v_sLog_Key;
...
//	函数功能：装载站址信息
//------------------------------------------------------------------------------------------------------			
void CCon_Station_Map::My_LoadData_Station_Data(CString v_sSql)
{
...
//-------------------------------------------------------------------------------------------------
void CCon_Station_Map::My_Station_Select(CString v_sTop, CString v_sLeft,CString v_sBottom, CString v_sRight)
{
	CString		str;
...
//	函数功能：事件映射【浏览器控件--函数】
//=================================================================================================
BEGIN_EVENTSINK_MAP(CCon_Station_Map, CFormView)
	//{{AFX_EVENTSINK_MAP(CMFCHtmlDlg)
	ON_EVENT(CCon_Station_Map, IDC_EXPLORER, 259 /* DocumentComplete */, OnDocumentCompleteExplorerMain, VTS_DISPATCH VTS_PVARIANT)
	//}}AFX_EVENTSINK_MAP
END_EVENTSINK_MAP()
...
//	函数功能：时间响应
//-------------------------------------------------------------------------------------------------
void CCon_Station_Map::OnDocumentCompleteExplorerMain(LPDISPATCH pDisp, VARIANT FAR* URL) 
{
...
****************************************************************************************************************/
HRESULT STDMETHODCALLTYPE CCon_Station_Map::GetTypeInfoCount(UINT *pctinfo)
{
	return E_NOTIMPL;
}
...
Description: 不用实现，直接返回E_NOTIMPL
****************************************************************************************************************/
HRESULT STDMETHODCALLTYPE CCon_Station_Map::GetTypeInfo(UINT iTInfo, LCID lcid, ITypeInfo **ppTInfo)
{
	return E_NOTIMPL;
}
...
****************************************************************************************************************/
HRESULT STDMETHODCALLTYPE CCon_Station_Map::GetIDsOfNames(REFIID riid, LPOLESTR *rgszNames, UINT cNames, LCID lcid, DISPID *rgDispId)
{
...
****************************************************************************************************************/
HRESULT STDMETHODCALLTYPE CCon_Station_Map::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid,
...
Description: JavaScript拿到我们传递给它的指针后，由于它不清楚我们的对象是什么东西，会调用QueryInterface来询问我们“你是什
			 ...
			 所以只能这样来回答它的询问
****************************************************************************************************************/
HRESULT STDMETHODCALLTYPE CCon_Station_Map::QueryInterface(REFIID riid, void **ppvObject)
{
...
****************************************************************************************************************/
ULONG STDMETHODCALLTYPE CCon_Station_Map::AddRef()
{
	return 1;
}
...
****************************************************************************************************************/
ULONG STDMETHODCALLTYPE CCon_Station_Map::Release()
{
	return 1;
}

DWORD CCon_Station_Map::GetProcessID()
{
	return GetCurrentProcessId();
}

void CCon_Station_Map::ShowMessageBox(const wchar_t *msg)
{
	CString v_sPara;
	CString content;
...
void CCon_Station_Map::GetStationBounds()
{
	int				v_iID = 0;												//KPI数量
