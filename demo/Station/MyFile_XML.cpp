...
CString CMyFile_XML::Read_XML_PM_Row_Add(CString v_sEnd)
{
	...
	int			v_iLen;													//临时变量
	double		v_dSum=0;												//临时变量

	map_C_Data					v_mChild_Data;							//容器：map_Child_Data
	map_C_Data::iterator		v_pChild_Data;							//迭代器：map_Child_Data
	...
	{
		mString_String::iterator v_pName = v_mName.find(v_sType_Name);	//查找：Counter序号【2】
		if (v_pName!=v_mName.end())										//找到？？？
		{
			v_pChild = v_cObject_Child.v_mChild.find(v_pName->second);	//查找：列名称？？？
			if (v_pChild==v_cObject_Child.v_mChild.end())				//没找到？？？
			{
				v_cObject_Child.v_mChild.insert(pair<CString,map_C_Data>(v_pName->second,v_mChild_Data));	//增加容器
				v_pChild = v_cObject_Child.v_mChild.find(v_pName->second);		//查找：列名称？？？
			}
			if (v_sObject.Left(4)!="抚顺")								//为抚顺小区？？？
			{
	...
	while(v_fRead.ReadString(v_sStr))									//数据读取：一行
	{
		...
		if (v_iLen>=0)													//找到？？？
			break;														//退出
		//获取：列名称子项【<SN>CONTEXT.AttRelEnb.RNL</SN>】
		...
		if (v_iLen>=0)													//找到？？？
		{
			...
			v_iLen=v_sStr.Find("</SN>",0);								//查找："</SN>"
			if (v_iLen>=0)												//找到？？？
				v_sKey = v_sStr.Left(v_iLen);							//获取：列名称子项
		}
		else
		{																//获取：数值【<SV>2</SV>】
			...
			if (v_iLen>=0)												//找到？？？
			{
				...
				if (v_iLen>=0)											//找到？？？
				{
					v_sValue=v_sStr.Left(v_iLen);						//获取：数值
					v_dSum+=atof(v_sValue);								//求和
...
bool CMyFile_XML::Read_XML_Alarm(string v_sFile_Name)
{
	...
	//获取：ObjType\Objject
	measData= measCollecFile->FirstChildElement("fileHeader");			//查询：fileHeader 
	if (measData==NULL)
		return  false;
	measData=measData->NextSiblingElement("alarmList");					//查询：alarmList【告警列表】
	if (measData==NULL)
		return  false;
