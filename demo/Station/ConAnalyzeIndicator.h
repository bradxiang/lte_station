...
// CConAnalyzeIndicator 窗体视图

class CConAnalyzeIndicator : public CFormView, public IDispatch
{
	DECLARE_DYNCREATE(CConAnalyzeIndicator)

protected:
	CConAnalyzeIndicator();													// 动态创建所使用的受保护的构造函数
	virtual ~CConAnalyzeIndicator();

public:
	IHTMLDocument2*		m_pIHTMLDoc;									//

	enum { IDD = IDD_CON_ANALYZE_INDICATOR};
#ifdef _DEBUG
